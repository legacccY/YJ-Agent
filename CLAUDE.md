...
## 工具调用纪律（强制执行，防止级联取消 / 写入错误）
- **删临时文件用 PowerShell `Remove-Item`，绝不用 `rm`**：`rm` 不在权限白名单会被拒；被拒的调用若和其他工具放同一并行批次，会**级联取消整批**（满屏 Cancelled，看似"写入错误"实为根本没执行）
- **高风险调用单独串行发出**：可能被权限拒绝、可能失败、或有先后依赖的调用，不与其他调用混在同一并行批次
- **并行调用克制**：一批最多 3-5 个且彼此完全独立；反复 echo / 重复读同一文件的探针属浪费，禁止
- **文件写入分步、不重复验证**：Write 一次写完即成功（Edit/Write 失败会直接报错，无须读回确认）；长文件或多段改动用 Edit 按顺序逐次进行，一次一处，不并行写同一文件
- **后台进程验证一步到位**：启训练后用单个轮询脚本（sleep+检查写一个文件）确认，不连发十几个 echo/tasklist/nvidia-smi 探针
...
- 可主动提问获取所需信息
- **自动多 sonnet agent 并行**（强制，无须每次征求同意）：任务含多个彼此独立、无文件冲突的子任务时，主动同时分派多个 sonnet subagent 并行处理；每个 subagent 给完整冷启动上下文（文件路径 / 目标 / 禁止项）。
  - **例外——仍由主线亲自串行，绝不外包**：训练启停、HPC 提交 / 上传、危险删除（Remove-Item / kill 进程）等关键路径或实时操作。
  - 主线在 subagent 跑时不空等，继续推进关键路径活。
