...
- **quantimmu-bench / QuantImmuBench**（任务含 新抗原/neoantigen/免疫原性/immunogenicity/癌症疫苗/工具部署/PredIG/DeepImmuno/pTuneos/IMPROVE/NeoTImmuML/QuantImmu/pooling/fusion/rank-fusion；status=active **论文项目**，已升级论文 schema）：**权威框架=`project/meeting/QuantImmuBench/paper/QuanImmu-Paper-Outline.md`（袁老师定稿，QuantImmu 三步框架+30 工具+人鼠四数据+三重检验，投 Briefings in Bioinformatics）**。读档链：`00_README.md` → `01_STORY.md` → `02_ACCEPTANCE.md` → `paper/QuanImmu-Paper-Outline.md`（权威）→ `reference/GAP_ROADMAP_vs_outline.md`（需优化/实验增改清单）→ `DEPLOY_TRACKER.md` → `04_LOG.md` 最新 entry。数字真源=`analysis/*.csv`（Bash 核不信 md/Read）；袁 md 声称值标「待核」。袁老师协作项目，余嘉子任务=工具部署测试（已 17/30）+配合朱同学 pooling/徐伊琳 QuantImmu 组/数据组(鼠数据缺)。
...
> 5. **纯 CPU / 工具打分推理批跑（不占 GPU）→ `<gpus>` 填 0**：免疫原性工具批跑（Repitope Rscript / ImmuneApp / PRIME / predict 等）、分析/pooling/probe 脚本这类**不占 GPU** 的活，若命中训练检测器（脚本名含 probe/sweep/pilot/experiment 或走 sbatch）被拦，**别裸启也别改名绕过**——直接 `python tools/gpu_slot.py request <project> <host> 0 [note]`。占 0 卡 → 恒 `GO`（`free>=0` 永真）、绝不挤正在跑的、hook 见 starting 条目放行、**不记 training friction**，跑完照常 `release`。这是登记而非拍板：CPU/推理活也进调度器账，只是占 0 卡。（工具推理批跑本就非训练，但仍走调度器记账以防混入真占卡的活。）
...
> - 调研→`researcher`、设计实验→`planner`、红队前提/设计/claim→`skeptic`、分析结果→`analyst`、写章节→`writer`、核数→`verifier`、找漏洞→`reviewer`。能并行就一批 3-5 个 opus 扇出。
...
**编排模型 = orchestrator-worker**：主线 = Opus（决策/写作/训练管控 + 拆活派单），**工人一律 Opus**（2026-06-27 用户拍板：因上下文/质量问题全部 agent 升 opus，不再用 sonnet 当工人）。主线在工人跑时不空等，继续推关键路径。
...
**自动多 agent 并行**（无须每次征求同意）：任务含多个彼此独立、无文件冲突的子任务时，主动同时派多个 opus agent，每个给完整冷启动上下文（路径/目标/禁止项）。
...
- **Subagent 扇出**（Task 工具派 `.claude/agents/` 角色）：子任务彼此独立、只需结果回汇 → 一批 3-5 个 opus 并行（Anthropic 研究系统标准：lead 派 3-5 worker）。
...
- 推一轮实验「设计→出结果」：`planner` 设计矩阵 → `skeptic` 红队设计（0 致命即过，有 🔴 回 planner 修）→ `coder` 并行实现各 config（无文件冲突可多 opus）→ **🛑（拍板）主线 `/loop /run-experiment` 跑** → `analyst` 解读+出图 → `verifier` 核关键数字。（一键 `/experiment-cycle <project>`）
