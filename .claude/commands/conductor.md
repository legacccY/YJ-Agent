...
> ## ⚡ 冷启动 30 秒上手（先敲命令，别先读完全文、更别手搓协调方案）
> ```bash
> python tools/pipeline.py list                 # 有没有在跑的图？
> python tools/pipeline.py status <project>     # 有 → 看全貌
> python tools/pipeline.py next <project>       # 卡在哪棒 / 拍板点 / 全完成
> python tools/pipeline.py init <project> -t experiment   # 没有且活够大 → 建图
> python tools/pipeline.py ready <project>      # 现在能派哪些棒（同组并行）→ 派对应 agent
> python tools/pipeline.py done <project> <棒>  # agent 回来 → 标完成，自动解锁下一棒
> ```
> 一篇多窗：各窗 `claim <project> <棒> <窗名>` 领不同棒；`ready --free` 看剩哪些可领。
> **遇到「协调/多窗/排活」需求 = 先跑上面命令，禁止从零手设计方案**（那正是本工具要替代的）。
...
## 🔍 检测验收 + 完成线停下（铁律，2026-06-20 用户拍板加固）

> 多窗协作两大反模式根治：①窗自报「干完了」主窗就信 ②窗干嗨了冲过完成线往下个棒推。

### A. 主窗检测验收 = 不信自报，读磁盘核（标 `done` 前强制）
任一棒 agent/窗回报「完成」→ **主窗禁止直接 `done`**，先从磁盘检测真产出再标：
1. `git status --short` + `git diff <该棒 territory 文件>` → 看它**真改了啥**（不是它说改了啥）。
2. **比对该棒 DoD**（`PLAN/WINDOW_TASKS.md` 或节点 desc）：每条完成线逐项对上没。
3. **代码棒必跑真验**：`pytest` + 关键 import/子类/烟测（非 mock），数字棒 Bash/Grep 核 csv（不信 Read）。
4. **抓越界**：改了 territory 外的文件？碰了拍板点文件（ACCEPTANCE 阈值/STORY）？→ 停下报，不默许。
5. 全过 → `done <node> --out "检测到的真产出+验法"`；没过 → `block` 回修。
> 口诀：**✓ 只发给「读 diff + 跑验 + 对完成线」三关过的棒**，自报不算数。winC/winD 已按此验（子类+pytest+5 TODO 逐条核），winA 改 ACCEPTANCE 本体被检测抓出=拍板点。

### B. 各窗完成线停下 = 到线 done，不冲下个棒
派单/开窗 prompt **第一行强制**：「先读本棒 DoD 完成线，干到线即 `done` 并**停**，不自动接下个棒」。
- 完成线=`PLAN/WINDOW_TASKS.md` 对应节点（无则节点 desc）。无 DoD 的棒先补 DoD 再派。
- 碰拍板点（gate 棒/改阈值/HPC/投稿）→ 硬停报主窗，**绝不自己越**（gate 棒 `ready` 返 rc=10 已挡一层，人也守一层）。
- 窗只碰本棒 territory；想干别棒先回主窗认领，不顺手做。
> 根治「Claude 推着用户往前走」：完成线是刹车，写死在磁盘 + prompt 双保险。
