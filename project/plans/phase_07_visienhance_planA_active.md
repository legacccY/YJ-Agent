...
|---|---|---|---|---|
| 2026-05-16_visienhance_s1_v0 | visienhance_s1.yaml (1.7M) | PSNR 25.55 / SSIM 0.9535 | ❌ done but inadequate | 容量不足（误判，实为裁剪 bug）|
| 2026-05-24_visienhance_s1_planA | visienhance_s1_planA.yaml (~15M) | PSNR 25.53 @ep42 | ❌ 卡死，已弃 | **9× 容量同 PSNR → 容量证伪**；真因 = 退化管线随机裁剪 |
| 2026-05-29_visienhance_s1_planA_nocrop | visienhance_s1_planA_nocrop.yaml (~15M, medium-only) | PSNR **28.01** @ep15 (训到 ep16 收工) | ✅ 验证成功 | 去裁剪后突破旧 25.5 死点，oracle 上界 37.49，待续训到收敛 |
| 2026-05-30_visienhance_s1_planA_nocrop_resume | 同上 (resume ep17→56) | **E1 PASS**：PSNR 32.5(每图均值,val)/32.74(test)，SSIM 0.946；训练日志 28.97=聚合MSE定义 | ✅ 收敛达标 | ep44–56 聚合 PSNR 平台；PSNR 两定义差=log非线性(已复现both)；无须 Plan B；`results/visienhance_nocrop_e1*.json` |

---

## Blockers

- ~~PSNR 卡 25.5~~ → 已根治（会话 8）：退化管线 `degrade.py` 的随机裁剪致像素错位，去裁剪后解除。

---

## 🔑 会话 8 关键决策：裁剪 → query-channel（必须回写主文档）

- **诊断结论**：PSNR≥30 卡死 = `apply_random_crop`（ratio 0.75-0.89）使降质/原图像素错位，强迫模型 hallucinate（违反 R8），非容量、非 loss 问题。证据链见 `scripts/diag_{visienhance_ceiling,degradation_decomp,crop_killer,nocrop_ceiling}.py` + PROJECT_LOG 2026-05-29。
- **设计决策**：增强任务只处理**像素对齐的可逆退化**（亮度/对比度/色偏/模糊/JPEG）；**取景错位/裁剪归 Theorem 2 的 query-for-retake 通道**（强化 Claim 3 闭环）。
- **数据**：无裁剪配对在 `data/paired_dataset_nocrop/` + `data/quality_labels_nocrop.csv`（medium 已全量；light/heavy 待生成）。原 `data/paired_dataset/` 保留可回滚。
- **待回写**（会话 9）：STORY_FRAMEWORK §4 Claim 2/3 + ACCEPTANCE E1 阈值确认 + 本 plan W1 任务勾选。
