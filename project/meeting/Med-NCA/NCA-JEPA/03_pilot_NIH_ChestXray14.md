...
> 最后更新：2026-06-15。状态：**计划已定，pilot 待用户拍板开跑**（训练启停归用户，串行红线）。
...
| 臂 | predictor | 步数 | 验什么 |
|---|---|---|---|
| **A0** | ViT predictor（I-JEPA 原版） | 1 | 管线 sanity + 下游上限。A0 学不动=管线坏，与 NCA 无关 |
...
**Pillar 3 — 独有能力**
- ② 随时推理：Q(k)=第 k 步提前终止 vs 满步的 cosine；**Q(8) ≥ 0.7·Q(64)**
...
- `results/pilot_*.csv` — 每臂逐 epoch 轨迹 + 塌缩指标（std/rank/KoLeo）+ 下游探针 AUROC
- 稳定性三联图 — loss / 塌缩 canary / rollout 残差（A1 vs A2）
- anytime 曲线 — Q(1,2,4,8,16,32,64)
