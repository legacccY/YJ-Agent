...
\textbf{(3)} We trace the instability to a concrete and transferable
mechanism: NCA's stochastic \emph{fire mask} (50\% per-step cell dropout,
$\times 64$ steps) draws from a CUDA RNG stream that \texttt{numpy}/\texttt{torch}
CPU seeding does \emph{not} control. Whether a run lands in a converging or
a diverging basin is decided \emph{at epoch 1} by this uncontrolled
randomness --- the \emph{same} seed (42) produced an epoch-1 loss of $1.25$
(converged) in one run and $4.33$ (diverged) in another. A
\emph{mathematically equivalent} CPU$\rightarrow$GPU change to the same
sampling collapses training from $0.672$ to $0.0$. We additionally
...
\begin{enumerate}[leftmargin=1.4em, itemsep=2pt]
  ...
  \item A precise root-cause for the instability: the convergence basin is
        fixed at epoch~1 by an \emph{uncontrolled CUDA RNG stream} feeding
        the fire mask; CPU seeding cannot lock it (same-seed runs diverge),
        and a mathematically equivalent CPU$\rightarrow$GPU change collapses
        training entirely (Section~\ref{sec:rng}).
...
two levels this is a long chain of Bernoulli draws applied to a
\emph{recurrent} update; small differences in the realised mask sequence
compound. Where these random numbers are drawn from --- and whether that
source is seed-controllable --- turns out to determine reproducibility
...
\subsection{Root cause: an uncontrolled CUDA RNG stream}
\label{sec:r2root}

The decisive observation is that \textbf{the same seed produces different
basins}. Seed 42 gave an epoch-1 loss of $1.25$ (and converged) in run
1435378, but $4.33$ (and diverged) in run 1436781 --- same code, same
config, same \texttt{numpy}/\texttt{torch} CPU seed. CPU seeding therefore
does not control the trajectory.

The uncontrolled source is the fire mask. The $0.5$ Bernoulli gate is drawn
on the GPU each step; CUDA's RNG stream plus non-deterministic kernels are
\emph{not} pinned by \texttt{np.random.seed} / \texttt{torch.manual\_seed}.
Each run realises a different mask sequence; over $64$ recurrent steps on
the aggressive $256^2$/ch32 configuration this is amplified enough to fix,
on epoch~1, whether the optimiser falls into a converging or a diverging
basin --- and, because the recurrence is always one bad draw away from a
runaway, to trigger a cliff at any later epoch. The mild Hippocampus regime
($64^2$, 16 steps) never exhibits this; Prostate sits on a numerical
stability boundary where the fire-mask noise is decisive.
...
% ------------------------------------------------------------
\section{Reproducibility Fragility: the RNG, Three Ways}
\label{sec:rng}

The Prostate instability is one face of a single transferable finding:
\emph{NCA training reproducibility is hostage to its randomness source.}
We have three escalating pieces of evidence, weakest to strongest.

\paragraph{(1) A ``free'' optimisation breaks it (controlled, single pair).}
...
official CPU-rand variant trains to $0.672$ (Figure~\ref{fig:rng}). This is
a clean controlled comparison and the reason our protocol forbids even
``equivalent'' re-implementations.

\paragraph{(2) The same config diverges across runs.}
...
epochs (185, 99). Re-running the exact same setup is not a reliable way to
get the paper number.

\paragraph{(3) The same \emph{seed} diverges (strongest).}
The 9-seed sweep (Section~\ref{sec:r2}) shows that seeding does not even
make the trajectory repeatable: seed 42 converged in one run and diverged
in another, and $0/9$ seeds survived. The randomness that matters is below
the level the seed controls.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig_rng_fragility.pdf}
\caption{\textbf{Fragility face (1): the controlled pair.} On Prostate
(identical config, RNG source only) a mathematically equivalent
CPU$\rightarrow$GPU change to fire-mask sampling collapses training from
$0.672$ to $0.0$. The Hippocampus bars converge but differ in configuration
too (ch16/64-step vs.\ ch32/16-step), so they are shown only as a
forgiving-regime reference, not a controlled RNG test.}
\label{fig:rng}
\end{figure}

\paragraph{Takeaway.}
NCA's iterated update is acutely sensitive to its randomness source. Any
``equivalent'' change to the RNG device or ordering can silently break
reproduction near a numerical-stability boundary, and on the aggressive
Prostate configuration even \emph{nothing but the RNG} suffices.
\emph{Reproduce NCA with the official random path, expect run-to-run
variance, and do not trust seed-level repeatability of training.} This is
the empirical justification for our zero-deviation rule.
...
in a randomness source that standard seeding does not control. Reproduction
studies that report a single converged run --- as the public artifact
implicitly does --- can mask this entirely.

\paragraph{Two faces of the same stochasticity.}
The inference variance that powers Med-NCA's attractive built-in quality
metric (Section~\ref{sec:behaviour}) is the \emph{same} fire-mask
randomness that destabilises Prostate training (Section~\ref{sec:rng}).
The mechanism the paper sells as a feature is, on the harder task, a
liability --- a tension worth flagging for anyone building on NCA.
...
propose no modelling change. A principled stabiliser (deterministic
fire-mask RNG, gradient clipping, or multi-modal input) that makes Prostate
...
partial run at $0.672$. We trace this to an uncontrolled CUDA RNG stream
feeding the fire mask: the convergence basin is fixed at epoch~1, the same
seed can converge or diverge, and a mathematically equivalent
CPU$\rightarrow$GPU change collapses training entirely. The transferable
lesson is that NCA reproduction demands fidelity down to the RNG stream and
that seed-level training repeatability cannot be assumed. We release
