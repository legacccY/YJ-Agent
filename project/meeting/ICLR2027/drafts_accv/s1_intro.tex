...
networks, vessels).
Crucially, none of these families \emph{preserves the diagnostic signal under a guarantee}: generic
enhancement optimises perception, not diagnosis. Gradient-level repair~\citep{gradprom2025} treats the
symptom during training; we instead bound the information loss directly and decide, per input, when
enhancement should be withheld in favour of re-acquisition.
...
bound.} Our core methodological contribution is a feature-level diagnosis-preserving objective
(DP-Loss): rather than optimising perceptual fidelity, it constrains the enhancer \visienhance to
retain the information a frozen diagnostic probe relies on, so that enhancement does not erode
diagnostic content within a moderate-degradation window. We derive a Pinsker-optimal
...
$p=2.3\times10^{-45}$), and against six restoration baselines \visienhance preserves diagnosis better
on every one\todo{R1 核：fine-tuned baseline 公平比数字补强后升级此句口径}.
