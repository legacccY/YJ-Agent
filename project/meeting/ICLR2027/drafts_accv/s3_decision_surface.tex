...
Before introducing diagnosis-preserving enhancement (Section~\ref{sec:enhance}) and the query-for-retake agent (Section~\ref{sec:agent}), we motivate \emph{why} quality-aware triage is needed at all. This section is a motivation chapter, not a standalone contribution: it provides the empirical groundwork that licenses the two methods that follow. We describe a two-axis decision surface over degraded dermoscopy: a \emph{reliability} axis led by diagnostic AUC across a per-dimension~$\times$~per-severity grid, and a \emph{recoverability} axis recording whether deterministic enhancement recovers the lost signal at each point. The surface is purely descriptive and serves only to delimit \emph{when} enhancement is licensed and \emph{when} re-acquisition is the safer route; it occupies an angle the per-dimension ECE/QCTS analysis of prior work (\citep{ANON-BMVC}, \texttt{tab:perdeg}; cited, not reused) does not cover, in that we lead with diagnostic AUC rather than calibration error, sweep severity, and add a completeness dimension.
...
Read together (Fig.~\ref{fig:c0-surface}), the two axes supply the motivation for the methods that follow: they expose a moderate window where enhancement demonstrably helps (Section~\ref{sec:enhance}), and isolated triggers where it should yield to re-acquisition (Section~\ref{sec:agent}). We treat the surface as the diagnostic substrate that justifies quality-aware triage rather than as a result claimed in its own right. The surface presupposes a per-input degradation estimate, obtained here from a five-head quality assessor (VisiScore; mean PLCC $0.924$ / SRCC $0.895$ against held-out degradation labels) but agnostic to its source: any per-input quality signal, whether learned, no-reference IQA, or known corruption severity, can be substituted.
