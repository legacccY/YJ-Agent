...
a quality flag in isolation. Adjacent lines that might appear to subsume it ---
active perception and next-best-view planning, active sensing and active feature
acquisition, and human-in-the-loop recapture --- each select a sensing or measurement
action, but none couples that decision to a per-input diagnostic-risk estimate with a
local risk bound, and so none falls within the risk-bounded, diagnosis-coupled routing
framework we study here. We make no claim that this routing raises net benefit globally;
\S\ref{sec:exp-dca} reports where it does and does not help.
