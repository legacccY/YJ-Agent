...
% ═══════════════════════════════════════════════════════════════════════════════
\subsection{Proposition 3 (Enhancement Entropy Reduction)}\label{app:a2-prop3}

\begin{proposition}[restated]\label{prop:entropy-full}
...
first term strictly negative. The drift $\Delta\Hent_\mu$ is controlled by
Assumption~\ref{as:a4}: Lemma~\ref{lem:dp-full} gives
...
salvage band gives \eqref{eq:p3-full}.
\end{proof}

\paragraph{Tight constants (non-vacuity).}
...
whereas the residual $2\beta\sqrt{\epsilon}\approx 2\cdot0.735\cdot\sqrt{0.05}\approx 0.33$ nats
($\beta=M\Lq/\sqrt{2}\approx 0.735$ for $K=2$, $\epsilon=0.05$). The gap exceeds the residual by
$\sim\!80\times$, so \eqref{eq:p3-full} is non-vacuous in our regime.
...
$\Lq$-Lipschitz classifier,
\begin{equation}
\|\hat{p}_{\mathrm{enh}}-\hat{p}_{\mathrm{ref}}\|_{\TV}
= \big\|\E[q_\theta(\cdot\mid\Zenh)-q_\theta(\cdot\mid\Zref)]\big\|_{\TV}
\leq \Lq\,\E\|\Zenh-\Zref\|_2 \leq \Lq\sqrt{\epsilon/2},
\label{eq:l3-2}
\end{equation}
absorbing the $O(\sigma(\qbar))$ effective latent radius into $\Lq$.
...
\leq M\,\Lq\sqrt{\epsilon/2} + O\!\big(\sqrt{\epsilon}\log(1/\sqrt{\epsilon})\big),
\label{eq:l3-3}
\end{equation}
...
\label{eq:l3-4}
\end{equation}
which is \eqref{eq:l3-full}. The $\sqrt{\epsilon}$ rate is Pinsker-optimal: a linear-$\epsilon$
bound would require a $\chi^2$-divergence assumption ($\chi^2\leq\epsilon$ in place of KL;
Appendix~A2.2.5).
\end{proof}
