...
only through decision-curve analysis and triage simulation (\cref{sec:exp-dca}); an LLM-as-judge
audit is specified as a protocol (Appendix~\ref{app:a19}) but not yet run, under the disclaimer in
Appendix~\ref{app:a23}, and we make no claim of real-time reader comparison. (iv) Salvage behaviour is bounded by Theorem~\ref{thm:agent} with thresholds
...
\textbf{Response.} We treat both points as first-class. (i) The benchmark itself spans a
controlled degradation taxonomy whose quality-stratified gap to \itb-LQ is reported explicitly
rather than pooled away, and the reliability~$\times$~recoverability decision surface
(\cref{sec:exp-surface}) quantifies, per dimension and per severity, exactly where real-world
degradation breaks diagnosis. (Evaluation on a dedicated in-the-wild smartphone subset, e.g.\
ISIC~2024 SLICE-3D, and cross-dataset transfer are left to future work and are not claimed here.)
(ii) Out-of-distribution inputs are \emph{foregrounded, not hidden}: severely degraded cases on
which enhancement is unsafe are routed to query-for-retake rather than diagnosed, the safety
boundary we analyse in \cref{sec:discussion}. (iii) The abstract and \cref{sec:intro} scope our
claims to dermoscopy quality-awareness, not universal quality awareness.
...
Concretely: the quality-conditional bottleneck (\cref{app:a1}) contributes an attention-drift bound
...
Corollary~\ref{cor:composite} recovers the prior post-hoc method as a degenerate case with an
...
plus ABCD-rule proxy detectors are specified to quantify any newly-introduced structure on a
held-out enhanced set (protocol in Appendix~\ref{app:a19}); the audit has not yet been run, and its
rates are deferred to the camera-ready rather than reported here. (iv) \emph{Decision}
