...
($32.74$ with FiLM versus $33.06$ without\todo{VERIFY: no-FiLM 33.06 源文件 e1\_film\_ablation.json 丢失（verifier 会话42 无源）；找 HPC .out 或重跑 e1 ablation 确认后方可入稿}), so FiLM is \emph{not} a fidelity mechanism --- yet it
...
The deterministic enhancer attains per-image PSNR $32.74$\,dB with FiLM (SSIM $0.91$\todo{VERIFY: 现存 visienhance\_nocrop\_e1.json 报 SSIM 0.946 而非 0.91，口径差异待核}, $n=19878$\todo{VERIFY: 现存 json 报 n=6626（test split）而非 19878，确认是否三档 severity 合并口径} held-out
...
(\cref{sec:exp-setup}), the caliber used for both the fidelity gate and the E10 comparison.
