...
($32.74$ with FiLM versus $33.06$ without
% E1 paired source RECOVERED (session 44, 2026-06-18) from HPC job 1442290.out (exit=0, clean):
%   with-FiLM perimg PSNR=32.743 (ckpt stage1_planA_256), no-FiLM=33.064 (ckpt stage1_planA_256_noFiLM), both n=19878.
%   Artifact restored at project/results/e1_film_ablation.json; raw log at meeting/ICLR2027/_e1_recovery/1442290.out.
), so FiLM is \emph{not} a fidelity mechanism --- yet it
...
The deterministic enhancer attains per-image PSNR $32.74$\,dB with FiLM (SSIM $0.91$, $n=19878$
% Caliber cross-checked (session 43): n=19878 / SSIM~0.91 is the e1_film_ablation harness,
% corroborated by e1_v6.json (n=19878, ssim 0.9094) and e1_v7.json (n=19878, ssim 0.907) under
% the same caliber; the earlier "nocrop n=6626 / SSIM 0.946 mismatch" flag was a false alarm
% (nocrop_e1.json is a separate test-split harness, not this table's caliber).
held-out
...
A per-degradation analysis (\cref{fig:e2-perdim}) shows the enhancer is strongest on brightness
($37.68$\,dB) and blur ($35.82$\,dB), weaker on colour-shift ($33.77$\,dB, below the $35$\,dB target), and --- notably ---
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{../../report/figures/fig_e2_perdim.pdf}
  \caption{\textbf{Per-degradation enhancement: PSNR gain and enhanced SSIM by corruption axis
  (E2).} \textbf{(a)} PSNR \emph{gain} (enhanced $-$ degraded; source
  \texttt{results/e2\_perdim.csv}). The gain is largest where the corruption is most severe
  (brightness degrades the input to $13.7$\,dB, so enhancement recovers $+24.0$\,dB to an absolute
  $37.7$\,dB), and small or negative where the degraded input already starts high: blur
  ($36.7\!\to\!35.8$\,dB, $-0.8$) and contrast ($32.3\!\to\!29.1$\,dB, $-3.2$). Contrast is the one
  axis where enhancement is genuinely \emph{harmful} --- the enhanced PSNR drops below the degraded
  input --- which is why the policy routes contrast-degraded inputs to query-for-retake rather than
  enhance. \textbf{(b)} Enhanced SSIM stays high on every axis ($\ge 0.92$).}
  \label{fig:e2-perdim}
\end{figure}
