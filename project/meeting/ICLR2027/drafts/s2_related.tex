...
\emph{by construction} (the feature-level DP-Loss of \S\ref{sec:enhance},
...
A complementary line of work in medical image-quality assessment flags or
instructs re-capture from an image-quality classifier. In teledermatology,
TrueImage~\citep{vodrahalli2021teledermatology} flags low-quality submissions and
prompts the user to re-photograph the lesion, and explainable IQA for
teledermatological photography~\citep{jalaboi2023explainable} similarly scores
capture quality to drive re-photography; in point-of-care ultrasound, a
closed-loop lung-ultrasound IQA system~\citep{zhao2020lungus} guides the operator
to re-acquire frames of sufficient quality. These systems recognise that the right
action under poor capture is sometimes to acquire a better image rather than to
diagnose the existing one, and so supply the clinical motivation for recapture.
In every case, however, the re-capture decision is made by an image-quality
classifier and is \emph{decoupled} from the diagnostic objective: the retake
decision is not tied to a quality-conditioned diagnostic-risk estimate, carries no
accompanying risk analysis, and does not sit inside a policy that also decides when
enhancement is safe.
