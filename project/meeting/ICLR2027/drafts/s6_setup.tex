...
collection (ISIC-2020), using a fixed train/validation/test split. The paired
diagnostic evaluations are run on the paired evaluation subset of the held-out
test split ($n=3627$, with $117$ melanoma-positive cases; the complete test
split contains $6{,}626$ images), and the diagnostic backbone is an
EfficientNet-B3 classifier (denoted B3) frozen across all evaluations so that
any change in diagnostic AUC is attributable to the input, not to retraining.
...
a centre crop to $224\times224$, and the B3 classifier. The moderate paired tier
composites blur, brightness, crop and colour shift; contrast ($\alpha$) is exercised
only in the per-dimension severity sweep below, not in the moderate paired protocol.
For the per-dimension
...
(Table~\ref{tab:e10}). All six baselines use official pre-trained weights
released by their respective authors, applied zero-shot to the dermoscopy
evaluation protocol.
