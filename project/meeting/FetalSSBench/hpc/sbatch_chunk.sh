#!/bin/bash
#SBATCH --job-name=fss_chunk
#SBATCH --partition=gpu4090
#SBATCH --account=shuihuawang
#SBATCH --qos=4gpus
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=4
#SBATCH --time=04:00:00
#SBATCH --output=/gpfs/work/bio/jiayu2403/fetalss/logs/chunk_%x_%j.out
#SBATCH --error=/gpfs/work/bio/jiayu2403/fetalss/logs/chunk_%x_%j.err

# M, DS 由 sbatch --export 传入 (method, dataset)
PYBIN=/gpfs/work/bio/jiayu2403/.conda/envs/yjcu124py310/bin/python
export FETALSS_DATA_ROOT=/gpfs/work/bio/jiayu2403/fetalss/data
export RESULTS_TAG=${M}_${DS}
export OMP_NUM_THREADS=4
cd /gpfs/work/bio/jiayu2403/fetalss/code

echo "=== chunk M=$M DS=$DS tag=$RESULTS_TAG start $(date) ==="
$PYBIN -c "import torch; print('cuda', torch.cuda.is_available())"
for R in 0.01 0.02 0.05 0.10 0.20; do
  for S in 0 1 2; do
    echo "---- $M $DS r=$R s=$S $(date +%H:%M:%S) ----"
    $PYBIN harness.py --method $M --dataset $DS --ratio $R --seed $S 2>&1 || echo "RUN_FAIL_${M}_${DS}_${R}_${S}"
  done
done
echo "=== chunk $M $DS done $(date) ==="
wc -l /gpfs/work/bio/jiayu2403/fetalss/code/results/results_${M}_${DS}.csv
