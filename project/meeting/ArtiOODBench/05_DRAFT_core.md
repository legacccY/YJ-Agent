...
**In-sample evaluation inflation (A-7, a methodological finding).** The size of this inflation is sharply localized to the projection-based detectors, which makes it a clean, mechanistic finding rather than a generic caveat. The gap Δ = (in-sample − held-out) AUROC is **+0.223 for ViM and +0.223 for Residual** (each falling from 1.00 to 0.777), but **|Δ| < 0.012 for all eleven other detectors**, which do not fit a low-rank ID subspace. This is the central mechanistic contribution of the paper: **projection-based / subspace-residual OOD detectors report spuriously near-perfect AUROC under in-sample evaluation on these benchmarks**, an effect that generalizes to any subspace-residual detector and that is invisible unless the evaluation protocol is reported. We present it as a finding and an evaluation-protocol pitfall, not as an implementation bug.
...
**A single factor—source distance—explains the spread.** Across the pairs for which we have an artifact-only reference (n=4), the held-out ViM AUROC tracks artifact-only separability almost exactly: Spearman ρ = 1.0 and Pearson r = 0.9995 (p = 0.0005). The more physically distinct the two sources, the more the detector can read; the more similar they are, the less. This relationship also yields an **honest negative example within the same modality**: NIH vs RSNA, two US chest-X-ray sources, has the lowest artifact-only separability (0.64) and a held-out ViM of **0.406, indistinguishable from chance**. This single point is decisive evidence *against* over-claiming: source leakage is not a universal property of the modality or the detector—it is a function of source distance, and it can vanish entirely when the two institutions are acquisition-similar.
...
We then deliberately stress-tested that very number, and it did not survive—so we report the correction in full. **Fourth**, an adversarial review flagged that ViM = 1.0 could be a trivial evaluation artifact rather than a genuine source covariate. **Fifth**, we wrote source-matched negative controls and confirmed exactly that: a same-source pair scored in-sample also yields 1.0, so the perfect separation came from the in-sample fit/evaluation overlap, not from source. **Sixth**, we re-ran the entire matrix under a corrected held-out protocol; the true ViM values are 0.406–0.997 (mean 0.777, >0.95 on only 2/7 pairs), so the strict A-5 bar **fails**, and we say so. **Seventh**, following the pre-specified fallback, we moved the load-bearing weight to the three results that *do* hold honestly: artifact-only localization (A-1/A-2, phenomenon), the in-sample evaluation-inflation mechanism (A-7), and the evaluation prescription (A-6). We state plainly that the headline was reframed twice and that the original "ViM = 1.0 perfect source leakage" claim was a self-corrected in-sample artifact; we do not pretend this was always the claim. The in-sample inflation is reported as a finding and a methodological contribution—a pitfall any practitioner can fall into—rather than as a defect we apologize for.
...
3. **Evaluate projection-based / subspace-residual detectors (e.g. ViM, Residual) under a held-out protocol, and always state the protocol.** Fitting and scoring on overlapping ID samples inflates these detectors toward near-perfect AUROC for purely geometric reasons unrelated to the OOD signal (here, a +0.223 AUROC inflation localized to ViM/Residual). A reported AUROC for such a detector is uninterpretable without knowing whether the fit set and the ID evaluation set were disjoint.
