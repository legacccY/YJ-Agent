...
8. **held-out 校正 + 承重据实再迁移**：用户拍板 held-out 重算（Entry 11）。held-out ViM 0.406–0.997（mean 0.777，>0.95 仅 1/7）→ A-5 v5 strict 门 **FAIL**，触预登记 FAIL 三档退路 #1。**承重再迁移到三贡献框架**：现象（L1，A-1/A-2 PASS，不变）+ 机制（A-7，**收窄 framing 见下**）+ 处方（held-out 协议 + sanity baseline + artifact 上界，A-6）。**in-sample 灌水写成 finding + 方法学贡献，不写成 bug 致歉。** source 信号据实写诚实量级（中等、source 距离驱动、含 NIH/RSNA 0.406 诚实负例），不再写「完美/普遍 source leakage」。**paper 必呈这条「v5 满分→reviewer 攻→负对照证伪→held-out 校正→承重据实迁移」时间线，证非 HARKing。**
...
- **source leakage 写诚实量级，不写「完美/普遍」（v6 红线）**：held-out 校正后 ViM 真值 0.406–0.997（mean 0.777，>0.95 仅 1/7），**不再写「ViM=1.0 完美 source leakage」**。写：source 信号真实可检（全 >> 同源 held-out chance ≈0.5 的中位）但**中等、source 距离是一个主要驱动因子（Spearman ρ=0.821 / Pearson r=0.955, n=7）、含诚实负例（NIH/RSNA held-out ViM 0.406≈chance）**。**【skeptic v6 改-2】** 7 对 artifact-only / held-out ViM 已补全：BraTS(1.000/0.997) HAM_fitz(0.991/0.938) VinDr_RSNA(0.907/0.772) NIH_VinDr(0.896/0.841) HAM_ISIC(0.816/0.689) ISIC_PAD(0.805/0.798) NIH_RSNA(0.640/0.406 诚实负例)。原 n=4 的 ρ=1.0/r=0.9995 因 4 点完美单调偶然概率≈8.3% 不算小，补到 n=7 后改「主要驱动因子之一」，不再写「单因子」。
- **A-7 in-sample 灌水写成 cautionary finding/机制贡献，不写成 bug 致歉、也不 claim「发现 held-out 才对」（v6 + skeptic 收窄红线）**：held-out 是 ViM 原文 + OpenOOD 既有标准协议（非本文发现）；本文量化的是一种**已知的** estimation/in-sample leakage 在 **N_id<D underdetermined 投影子空间 + 跨源 medical benchmark** 下被放大到 Δ=+0.223（ViM/Residual 1.00→0.78，其余 11 法 |Δ|<0.012）且**精确伪装成完美 source-leakage 结论（ViM=1.0，骗过投稿前自审）**。这个 N_id<D 放大效应 + masquerade 机制文献未专门打包过，是 cautionary 方法学贡献，比原「ViM=1.0 完美 source leakage」更契合 D&B。**绝不 claim「我们发现投影类需 held-out 评估」。**
