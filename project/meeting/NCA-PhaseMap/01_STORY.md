...
越过临界 update_rate（≈0.35–0.40 / fire_rate≈0.60–0.65）NCA 即塌缩到平凡背景解；临界尖锐（过渡宽 ≤0.10）、seed 稳定、与梯度幅度无关。社区一直只把 fire_rate=50% 当固定超参直接用，从没系统刻画过这条边界——本文首次。

> 2026-06-18 数值订正（用户拍板）：原「≈0.375 / 过渡宽 ≤0.05」过度精确——实测过渡区 ur0.30(1/3塌)→0.35(0/3塌)→0.40(3/3塌) 跨 0.10。改区间表述。临界点最终值待 Gate1 **no-clip 复测**（G5 带非官方 clip，见下措辞红线 clip 条）。

## 三支柱卖点

1. **尖锐可前验临界**（非模糊经验调参）：update_rate 越过 ≈0.35–0.40 即从"能学"断崖跌到平凡解，三重独立实验坐实（原 36cell + 单轴细扫 + 3 seed STABLE_SHARP）。⚠️ G5 三重实证带非官方 clip=1.0，临界点待 Gate1 no-clip 复测精化。
2. **塌缩 ≠ 梯度爆炸**（反社区直觉）：塌缩与 max_grad_norm 统计无关（r=0.238 p=0.16），高梯度反而存活（ur=0.30 grad=396 活）。社区从没真把"梯度爆炸"当 NCA 训练失败主因——本文厘清真正的因子是 update 稀疏度。⚠️ **r=0.238 是 clip=1.0 下测的，可能是 clip artifact（名义 grad 被裁到 1.0、与真实步长脱钩）→ 支柱2 命门，Gate1 腿② 必在 no-clip 下重测（见 ACCEPTANCE 🔴-6）。复核前此 claim 不定稿。**
