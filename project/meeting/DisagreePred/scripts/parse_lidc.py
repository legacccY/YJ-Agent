"""
parse_lidc.py — DisagreePred A1 方案甲：pylidc 解析 LIDC-IDRI，构建分歧标签 + patch

服务项目：DisagreePred，lever = A1 扩样坐实 + k=1/k≥2 消融
...
新增列 k_solo（A1 消融，skeptic 红队要求，2026-06-19）：
  - k_solo = "solo"   : k=1（仅 1 名医师标注，存在性争议最强）
  - k_solo = "multi"  : k∈{2,3}（多人标注后仍分歧，共识分歧）
  - k_solo = "agree"  : k=4（全体一致，disagree_binary=0）
  供「k=1 捷径敏感性」消融：验证 AUROC 不靠 k=1 单人标撑分
  消融用法：过滤 k_solo != "solo" 后只跑 k∈{2,3} vs k=4，看 AUROC 变化
...
def parse_lidc(data_root: Path, out_dir: Path = OUT_DIR,
    ...
    for scan in scans:
        ...
        for cluster_idx, cluster in enumerate(clusters):
            ...
            # 消融列 k_solo（skeptic 红队要求，2026-06-19）：
            #   "solo"  = k=1（单人标，存在性争议最强；潜在捷径风险）
            #   "multi" = k∈{2,3}（多人标后仍分歧，真实共识分歧）
            #   "agree" = k=4（全体一致，disagree_binary=0）
            # 用途：过滤 k_solo!="solo" 后跑 multi vs agree 消融
            #       若 AUROC 不降，说明模型不靠 k=1 撑分（消除捷径质疑）
            if k == 1:
                k_solo = "solo"
            elif k in (2, 3):
                k_solo = "multi"
            ...
            rows.append({
                ...
                "k_solo": k_solo,          # 消融列：solo/multi/agree
    ...
    fieldnames = [
        ...
        "disagree_binary", "disagree_entropy",
        "k_solo",          # 消融列：solo(k=1)/multi(k∈{2,3})/agree(k=4)
        "patch_path", "split",
    ]
    ...
    k_solo_dist = {}
    for r in rows:
        k_solo_dist[r["k_solo"]] = k_solo_dist.get(r["k_solo"], 0) + 1
    ...
    summary = {
        ...
        "k_solo_distribution": k_solo_dist,   # solo/multi/agree 分布，消融参考
        ...
        "ablation_note": "k_solo 列供消融：过滤 k_solo!='solo' 只跑 multi vs agree，验证 AUROC 不靠 k=1 撑分",
    }
    ...
    print(f"  k 分布: {k_dist}")
    print(f"  k_solo 分布（消融参考）: {k_solo_dist}")
    print(f"    solo(k=1)  = {k_solo_dist.get('solo', 0)} 个（单人标，存在性争议）")
    print(f"    multi(k∈{{2,3}}) = {k_solo_dist.get('multi', 0)} 个（多人标后仍分歧）")
    print(f"    agree(k=4) = {k_solo_dist.get('agree', 0)} 个（全体一致）")
