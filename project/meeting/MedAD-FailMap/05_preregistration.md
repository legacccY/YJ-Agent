...
> **PR 间一致性（reviewer 核）**：PR-1↔PR-7 闭合（detected 只对 iso=True 产读数）；PR-2↔§A.2 无涟漪；PR-4↔§C 不跨 family；PR-5↔§2 G1-d 统一；PR-7b↔续命后门铁律1/4 互相加固（连续化趋势用所有对实测、不止 HAM 单点）。Phase 0 既有口径零被动。
