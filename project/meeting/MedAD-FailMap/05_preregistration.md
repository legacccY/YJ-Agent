...
## F. Phase 1 / Gate1 预登记冻结（PR-1..7b，headline A 双臂外推有效性判据）

> 冻结日 2026-06-17。流程：planner 设计（06_plan）→ skeptic 红队（0 致命放行）→ reviewer 复裁（7 可冻 + PR-6 暂搁）→ 主线拍板冻结、留痕。**§A.1/§A.2/§B/§C/§E Phase 0 既有口径一律不动**（本节只加 Phase 1 跨集新特征/新对的口径，不改任何 Phase 0 确证检验定义或门槛）。判据全文见 `06_phase1_plan.md §2` 双臂机械判定 + 续命后门铁律 1-4。

| # | 冻结口径（reviewer 收紧后定稿） | 状态 |
|---|---|---|
| **PR-1** | 跨集 detected 语义 = **目标集异常子集内 P90 = y_fail**（集合无关通用规则，适用 CBIS/METS/未来候选），与 BraTS tumor-only 内 P90 同构。**HAM 已作废落负臂、不产 detected 确证读数，不作正臂样例锚定**。 | ✅ 冻 |
| **PR-2** | 跨集新特征环宽 = **相对环宽三档 {5/7.5/10%}**（mask 等效直径相对宽，敏感性闸；非单值），两端同 64² 坐标系。**Phase 0 `compute_contrast(dilation_px=3)` 绝对环宽冻结不动、不涟漪 §A.2**（pytest 已验未被动）。 | ✅ 冻 |
| **PR-3** | proxy-clf + scaler **零调参**：[size,contrast] clf BraTS fit / 目标 transform 不 refit；StandardScaler 同样 BraTS fit / 目标 transform；**Gate1 确证不用整图代理**（守 §3 铁律 + T6 退守留痕）。 | ✅ 冻 |
| **PR-4** | 多重比较 = Phase 1 跨集对成新 family **F-B'**，family 内 Holm（CI Bonferroni 调）；**F-B'（Phase 1）与 F-B（Phase 0 T6/T7/T8）两独立 family 不合并校正**；**F-B' 成员（几对 × SB1/SB2）须跑前列举冻死，不许跑完按结果增删对改 Bonferroni 分母**。 | ✅ 冻 |
| **PR-5** | 多 seed 聚合：≥3 seed；**worst 档 = seed 间最差 CI 下界**（定死，删「点估计 vs seed 间」二选一开口；与 §2 G1-d、判定表一致）。 | ✅ 冻 |
| **PR-6** | C4 排序方向 + reliability 权重 | 🟠 **暂不冻**，维持 §A.3 探索性；纳 C4 确证须**另走**「跑前冻方向+权重 + reviewer 复裁」独立预登记，不在 Gate1 双臂闸关键路径上 |
| **PR-7** | area-ratio 重叠**存在性检查**（G1-a 子条件）：BraTS 与目标集病灶/背景面积比分布须实质重叠。与 PR-7b 合并表述（PR-7 = 存在性，PR-7b = 阈值规则）。 | ✅ 冻（合 7b） |
| **PR-7b** | **iso 阈值确定规则（skeptic 唯一真残口，reviewer 三处收紧）**：① **两段阈值都钉死**——`brats_low_ratio_pct=25`（≤P25=0.0198 稀释 regime）**+** `min_overlap_frac=5%`（目标集 ≥1/20 样本落该 regime = iso=True，否则 False）；**二者均不得在看任一候选对（CBIS/IDRiD/METS）实测重叠率后调整**。② **非循环依据**：5% = 「目标集至少 1/20 样本落入 BraTS 稀释 regime 才算分布有实质交叠」的先验充分性论证，**不引用任何候选对实测分布反推**。③ **连续化趋势检验作 ② 判据主统计证据**——预登记「跨所有对（HAM/CBIS/METS/Gate2 held-out）重叠率↑ → 外推保留率↑单调」一条曲线，二分 iso 阈值仅用于 Gate1 双臂分诊（哪对进正/负臂），单调趋势撑判据成立（天然对齐 Gate2 held-out 盲测 + 缓解 n=1 点定线）。**时序清白留痕**：P25+5% 在 HAM/CBIS/IDRiD/METS 任何实测重叠率出炉前已定（04_LOG 2026-06-17 entry 为证，HAM 1.3% 实测在阈值定义之后才出）。 | ✅ 冻（收紧后） |
| **PR-7c** | **多灶性同构子条件（iso 第二维，单向占比门，同构复用 PR-7b area 维结构）**：iso=True 须**同时**满足 (i) PR-7b area-ratio 占比门 **AND** (ii) **n_components 占比门**——`brats_ncomp_regime_pct=75`（BraTS 自身 n_components P75 实测=3，参照系-intrinsic、与 area 维用 BraTS-P25 同构、零目标集依赖）**+** `min_ncomp_overlap_frac=5%`（目标集 ≥1/20 样本 n_components ≤3 = 该维 iso=True，否则 False）。n_components = 与 area-ratio 同 64² 网格连通域计数（skimage `label` connectivity=1）。**单向**（只问目标集是否落 BraTS 低焦 regime，与 area 维同结构；**不引「双向」非对称口径、不引自由参数 T、不用分布重叠系数 OVL 作二分门**——离散计数长尾 OVL 估计不稳，reviewer 复裁逮出）。任一维不过即 iso=False 落负臂。**OVL/Bhattacharyya（固化 bin，`results/distribution_overlap_*.csv`）退为 PR-7b③ 连续趋势 + A' 描述性支撑证据，非二分门。** ① **时序清白**：本子条件判据结构 + 两常数（P75=3、5%）在 METS/任何后续候选对实测 n_components 出炉前冻结（04_LOG 2026-06-17 entry + `ncomp_brats.csv` n=1948 median=2/P75=3 为证）。② **激发/检验切分（防 HARKing，对齐 STORY §七）**：IDRiD n_components 中位 136（~0% 样本 ≤3）为**激发本子条件的发现性观察**（motivating，类比训练集不计测试精度），**不计入 PR-7c 判据预言命中证据**；预言性检验落 METS（正臂）+ Gate2 held-out 盲测对。**反事实自缚**：若 METS/held-out 实测落 BraTS 低焦 regime 却外推塌、或不落却 PASS，按铁律5/前验门失灵处理 = FAIL，不得以「IDRiD 已激发」为由回溯救。③ n_components 维触发的 iso=False，其负向预言入判据证据**同受铁律4 约束**（需 ≥1 实测塌锚）。 | ✅ 冻（skeptic+reviewer 双闸收敛后） |

> **PR 间一致性（reviewer 核）**：PR-1↔PR-7 闭合（detected 只对 iso=True 产读数）；PR-2↔§A.2 无涟漪；PR-4↔§C 不跨 family；PR-5↔§2 G1-d 统一；PR-7b↔续命后门铁律1/4 互相加固（连续化趋势用所有对实测、不止 HAM 单点）；**PR-7c↔PR-7b 同构（两维同结构：BraTS 自身分位 regime 边界 + 5% 占比门 + 单向）↔铁律5（双维标签跑外推前钉死、禁单维回溯改判）**。Phase 0 既有口径零被动。

### F.iso-audit —— iso 门 reviewer 4 必修审计留痕（清白靠结构，不靠时序）

> **背景与状态**：iso 门在 ICLR 命门正臂攻坚期经历一次**统一重设计**（planner→skeptic 0 致命 PASS，2026-06-17）：统一 anatomy 分母 + area 维改 **双侧区间占比门**（band = BraTS 自身 [P33, P67] + 无参双侧守门 `inband ≥ 下方 AND inband ≥ 上方`）+ n_components 维沿用 PR-7c 单侧上界（≤P75=3, 5%）。该重设计因随后正臂落空、项目退守 MICCAI/MedIA，**未冻为正式 PR 取代上表 PR-7b/7c**；但其 reviewer 复裁要求的 4 条必修留痕属判据框架诚信记录（判据框架是 MICCAI 贡献之一），在此一并落痕，**进正文（本协议）而非只留 LOG**。本子段为审计留痕，不改上表 PR-7b/7c 的任何阈值结构或判据数值，也不动 §A.2。

**必修 1 — band 后设留痕（清白靠结构、非靠时序）**：双侧 band 门重设计**产生于 HAM/IDRiD 实测重叠率出炉之后**，故**不得**主张「band 阈值冻结早于实测」式时序清白。其清白证据是**结构性的**，由三条互相加固：(i) **无参双侧守门是真主力判据**（`inband ≥ above AND inband ≥ below`，零自由参数）；(ii) **band 宽度不承重**——8 组合实算证 band 取 [P33,P67] 还是 [P25,P75]（宽度 ±33%）对四集 verdict **零影响**（HAM/CBIS/IDRiD 全 False 各因 above/below 远超 inband，BraTS 自洽 True），band 仅作语义锚不作判据闸；(iii) **零候选集依赖**（边界全取 BraTS 自身分位）。三者使「band 事后收窄救 verdict」的 HARKing 残口被结构架空。写作纪律：诚实写「重设计产生于实测之后、清白靠结构非时序」，**禁写「band 冻在实测之前」**。

**必修 2 — 占比门非 OVL 系数当二分门**：iso 二分判定**只用 threshold + 占比门**（落 BraTS regime 的样本占比 ≥ 5%），**绝不用分布重叠系数 OVL / Bhattacharyya 作二分阈**。理由：离散计数（n_components）长尾分布的 OVL 估计不稳，且实证 n_comp OVL（HAM=0.498 / CBIS=0.506）跨 0.5 仅差 0.008 = 噪声刀刃，以 0.5 当门有反推 verdict 的循环嫌疑（reviewer 当年已驳回，skeptic 二次确认保留）。OVL/Bhattacharyya（固化 bin，`results/distribution_overlap_*.csv`）**退为 PR-7b③ 连续趋势 + A' 描述性支撑证据**，不进二分判定路径。（与上表 PR-7c 末段一致，此处对 area 维双侧门同样适用。）

**必修 3 — 反事实自缚句（带 FAIL 后果，已兑现）**：预登记自缚——**若正臂候选实测落 band 下方（= 真 iso=False），绝不放松守门救该正臂**。此自缚已在 LGG 上**真实兑现**：精确 skull-strip（256² + 腐蚀去颅骨，brain≈40% 对齐 BraTS 39.5%）后，LGG 独立例（15 患者 n=228）落 band 内 39.9% / 下方 44.7% / 上方 15.4% → `inband < below`（差 4.8 点）→ **iso_area=False**。尽管 4.8 点差落在 2D Otsu skull-strip 近似误差量级内、且 LGG 非 CBIS 那种干净 disjoint，仍**按机械守门字面接受 iso=False、不动门**——为这 4.8 点放松守门正是整套判据要防的 HARKing。其直接后果（开放数据填不出干净 ICLR 命门正臂 → 退守 MICCAI/MedIA）已被诚实承担，写入 STORY「正臂落空 = 开放数据系统性覆盖不到 dilution regime 的 benchmark 生态发现，只 claim 负向命中」。

**必修 4 — §A.2 对齐措辞（防量纲偷换）**：双侧 band 门取 BraTS 自身 [P33, P67] 的方法论与 §A.2「size/contrast 分位数三等分（33/67）」**同方法论（BraTS-only tertile）**，但**不是同一冻结值**——§A.2 分的是 size/contrast 两个协变量的三等分桶边界（用于 PC-A 分层），iso 门 band 分的是 area-ratio 这一几何同构量的分布边界（用于 Phase 1 双臂分诊），**量纲与对象不同**。写作时须表述为「沿用 §A.2 同一 BraTS-only 分位方法论」，**禁表述为「与 §A.2 同冻值/同阈值」**，避免量纲偷换。（PR-2↔§A.2 绝对环宽无涟漪结论不受本条影响。）

## 修订记录

- **2026-06-18 Phase 2 VAE 训练 nan 发散 → 用户拍板选 A：加 nan/inf 批次跳过守卫（robustness guard）—— 已知微偏离留痕**

  **现象**：Phase 2 seed-fill 中 VAE seed 1（epoch 60）和 VAE seed 2（epoch 10）均出现 `loss=nan` → `RuntimeError: CUDA illegal memory access`（`train_recon_ae.py:789 epoch_loss += loss.item()`）；VAE seed 42（早先跑）251 epoch 全程健康。

  **根因（coder 诊断，已核官方源）**：`vae_loss`（L257）计算 `lv.exp()` 及 `reparameterize`（L213）计算 `torch.exp(0.5 * log_var)` **均无 logvar 上界保护**。网络早期 logvar 可漂移至 float32 溢出，`exp(logvar)` → +inf，forward 输出 nan，backward 触发 CUDA illegal memory access。**已核 MedIAnomaly 官方源码**（`reconstruction/networks/base_units/blocks.py` VaeBottleNeck + `reconstruction/utils/losses.py` VAELoss + `reconstruction/utils/vae_worker.py`）：**官方同样无 logvar clamp、无 gradient clipping、无 KL warmup**——我们代码与官方完全对齐，nan 发散是官方配置下的 seed 数值不稳（seed 42 侥幸，seed 1/2 初始化路径更快触发上溢），属 (b) 非 (a)（即不是漏了官方已有的稳定化）。

  **用户拍板选 A（最小侵入）**：在训练循环每批 loss 计算后、`backward()` 前加 nan/inf 守卫——跳过该批次不 backward 不 step 不计入 `epoch_loss`，记 `n_skip` 计数，每 epoch 若有 skip 在 stdout 打印（便于诊断）。**只加守卫，绝不改 lr / beta / logvar clamp / gradient clip / loss 公式 / epochs 等任何超参或结构**（守复现纪律）。

  **改动**：`code/train_recon_ae.py` 训练循环 L769 附近，新增 `n_skip` 计数变量 + `if torch.isnan(loss) or torch.isinf(loss): n_skip += 1; continue` + epoch 末 skip 打印逻辑（共 +8 行，原有 backward/step/accumulate 位置不变）。

  **已知微偏离声明**：本守卫是相对官方 MedIAnomaly 实现的微偏离——官方 `vae_worker.py` 训练循环无任何 skip 逻辑。选 A 理由：① 最小侵入（不改超参/结构/loss）；② seed 42 已完成不需重跑，可与守卫保护下的 seed 1/2 平均 AUROC；③ skip 是 robustness guard 非调参行为，对最终 AD 分布影响可忽略。**若 seed 1/2 重跑后每 epoch skip 数量极大（如 >50%批次持续 skip），需升级报告主线评估是否彻底重新选 seed（选 B）**。

  **修改纪律遵守**：本条只加 robustness guard 并留痕，**不改任何 §A/§B/§C/§D/§E/§F 既有阈值/确证检验定义/Gate0 门槛**；VAE 结果仍属 D 节「探索性产出」（Phase 0 主跑 AE），本修复不改该分类。

- **2026-06-18 iso 门 reviewer 4 必修留痕补进正文（新增 §F.iso-audit 子段）—— 退守 MICCAI/MedIA 后判据框架诚信收口**

  承 2026-06-17 iso 门统一重设计（planner→skeptic 0 致命 PASS）reviewer 复裁要求的 4 条必修留痕，此前只在 04_LOG，本次**补进本协议正文 §F.iso-audit**（判据框架是 MICCAI 贡献之一，留痕须进文档非只 LOG）：必修 1 band 后设留痕（清白靠无参守门 + band 不承重 + 零候选依赖三结构，非时序）／必修 2 占比门非 OVL 系数当二分门／必修 3 反事实自缚句（LGG 落 band 下方 = 真 iso=False，绝不改门救，已真实兑现并承担退守后果）／必修 4 §A.2 对齐措辞（同 BraTS-only 分位方法论、非同冻值，防量纲偷换）。**本条只补留痕，不改上表 PR-7b/7c 任何阈值结构/判据数值，不动 §A.2/§A/§B/§C/§E。** iso 双侧 band 门重设计本身未冻为正式 PR（正臂落空、退守 MICCAI 后搁置），状态已在 §F.iso-audit 抬头注明。

- **2026-06-18 C3 偏相关 bug 修正留痕（Phase 0 主目录 `incremental_C3_partial_corr.csv`）—— verifier 揪出 → coder 定位 → 主线重跑（不覆写 frozen 原值，全审计痕保留）**

  **bug**：Phase 0 主目录冻结产物 `incremental_C3_partial_corr.csv` 的 `controlled_for` 字段为**空**。根因 = `incremental_stats.py:606-608` 默认 `--stratify-csv` 指向 `stratify_interact_ae.csv`（这是 3×3 交互的聚合桶表，**无 `filename` 列**）→ 与 per-image 特征表 join 失败 → `covariate_cols=[]`（控制变量集为空）→ 实际计算的 `partial_r` 是**无条件 Pearson 相关，而非控制 [size, contrast] 后的真偏相关**。即原 C3「控制 size+contrast 后残差偏相关」实质没控住任何协变量。

  **诚实定性**：因此 Phase 0 原 C3 的 `partial_r` 数值（例如 glcm_cluster_prom 原值 **0.5044**）不能作为「conspicuity 在已知 size/contrast 之外仍有增量信息」的确证读数——它只是无条件相关。本项目 PC-C（Pillar ③）的**承重柱据此改以 C4 risk-coverage（actionable selective-AD 校准）领证，不再押 C3 小 `partial_r`**（与 2026-06-18 双柱重铸后的 STORY/ACCEPTANCE 一致）。

  **修**：① 代码 `incremental_stats.py:606-608` 默认 `--stratify-csv` 改为 `stratify_per_image_ae.csv`（含真 `filename`/`size_px`/`contrast` 列）；② 重跑结果另存 `results/c3_corrected/`，**绝不覆写 Phase 0 原 frozen 文件**——buggy 原值原地保留供审计追溯（coder 重跑时权限正确拦下覆写冻结产物，主线改道另存）；③ corrected 真偏相关（真控 size+contrast）：glcm_cluster_prom partial_r = **0.3054**（旧 buggy 无条件值 0.5044）。其余 corrected 值（glcm_contrast / sigma_global / fft_spectral_entropy / cnr_proxy_otsu）见 `results/c3_corrected/`，Holm 校正后符号方向与显著性结论**不翻**——PC-C 承重在修正口径下仍站得住。

  **修改纪律遵守**：本条只**订正一个实现 bug + 另存 corrected 不覆写原值 + 留痕**，**不改任何 §A/§B/§C/§E 既有阈值/确证检验定义/Gate0 门槛**，也不改 C3 检验的统计定义（仍是 residualize on [size,contrast] 后 Pearson）。corrected 值供 STORY/ACCEPTANCE 回填 `[pending]` 占位时引用（写作侧另过 verifier 复核）。

- **2026-06-17 PR-7c 多灶性同构子条件冻结（§F 表新增）+ 06 §2 续命后门铁律5 —— skeptic 红队 → reviewer 复裁双闸收敛 + 主线拍板（用户给 METS PAT 同时放行，时序：冻结早于任何 METS n_components 实测）**

  **缘起**：IDRiD 眼底实测暴露——union area-ratio 14.8%>5% 过 area 维门，但几何是 136 微灶散布 vs BraTS 1-3 灶 → 单 area-ratio 不足判同构，iso 须二维（area-ratio AND 多灶性）。**这是 novelty 加强（geometric isomorphism 多维）非补丁**。

  **双闸收敛**：skeptic 判原拟「单边上限门 n_components≤T」🔴（与铁律2/4 冲突 + 把唯一正臂 METS 暴露在「调 T 救/杀臂」双输循环），fix=双向重叠门不引 T。reviewer 复裁「双向 OVL」🟠（实质循环滑进 regime 边界来源 + 「双向」非对称 + 离散计数 OVL 系数不稳），fix=threshold+占比门、单向、两常数钉死。**主线收敛**=直接同构复用已过双闸的 PR-7b area 维结构搬 n_components 维：BraTS 自身 P75（=3，参照系-intrinsic）作 regime 边界 + 5% 占比门 + 单向，OVL 系数退连续趋势/描述性证据不当二分门。同时满足两闸 + 科学（IDRiD ~0%≤3→该维 iso=False；HAM/CBIS 中位1→该维过靠 area 维判 False；METS 待实测）。

  **数字固化前置**（coder，165 pytest 绿）：`ncomp_brats.csv`（median=2/P75=3）+ `ncomp_ham.csv`（median=1）+ `distribution_overlap_*.csv`（钉死 bin，BraTS↔IDRiD n_comp OVL=0.001 极不相交）。订正 LOG 旧未固化 OVL/BC + BraTS「中位 1-3」→ median=2。**§A.1/§A.2/§B/§C/§E Phase 0 既有口径一律不动**（本条只加 Phase 1 iso 第二维，修改纪律：skeptic+reviewer 双闸 + 主线拍板 + 留痕齐备）。

- 2026-06-17 planner 初版冻结（A0 提交前）；主线采纳全部推荐拍板，落盘 `05_preregistration.md`。

- **2026-06-17 headline A 重铸 + Phase 1 / Gate1 PR-1..7b 冻结（§F 新增）—— skeptic 0 致命放行 + reviewer 复裁 7 可冻 + 主线拍板**

  旧 ② headline「裸可外推/对任意新图 predict」被自家 G1-a（BraTS→HAM 1.3% 重叠几何不同构）字面证伪 = claim 辖域写错。用户拍板路径 A：② 重铸为**外推有效性判据**（可外推当且仅当几何 regime 同构 + 同构性可 area-ratio 前验，双臂判定）。G1-a 负诊断翻身成判据正面证据。skeptic 二次红队（是否重开续命后门）= 0 致命放行（三道防护机械层切开旧偷换）；reviewer 复裁 PR-1..7b = 7 可冻（收紧措辞）+ PR-6 暂搁。本节 §F 落 7 条冻结口径。**§A.1/§A.2/§B/§C/§E 既有阈值/确证定义一律不动**（修改纪律：reviewer 复裁 + 主线拍板 + 留痕齐备）。STORY/ACCEPTANCE/06_plan 同步重铸（见各文件 + 04_LOG）。下一步：CBIS-DDSM mass 下载（kaggle awsaf49，CC-BY-SA-3.0，进行中）→ 跑 G1-a 验 iso 找正臂。
