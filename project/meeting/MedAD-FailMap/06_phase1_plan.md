...
> 5. **iso 双维标签跑外推前一次性钉死，禁按单维回溯改判**（PR-7c 引入 n_components 第二维后补，skeptic+reviewer 双闸 2026-06-17）：本铁律 = 铁律1（时序钉死）+ 铁律2（塌即 FAIL）在 METS（及任何正臂候选）**双维 iso 标签（area-ratio ∧ n_components）**上的实例化，**仅收紧不放松**。新增量 = **禁止按单维（多灶/area）事后拆分回溯改判整体 iso**——iso=True 对若塌，不得回头说「其实它多灶性/面积比不够 iso、所以不算正臂失败」，那是 PR-7c 多维后新开的偷换路径。METS iso 双维标签在跑其外推 AUROC 前由 PR-7/7b/7c 前验门固定写 05；冻后塌即按铁律2 处理（前验门失灵/FAIL），不得回溯洗负臂。
