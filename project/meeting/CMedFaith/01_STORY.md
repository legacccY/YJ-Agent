...
> **医学 RAG 系统里，大模型会生成看似专业、却不忠实于所检索医学证据的回答；现成 faithfulness 检测器在中文医学域上不可靠，而中文医学场景连评测这一缺陷的数据都没有。CMedFaith 建首个中文（+英文对照）医学 evidence-conditioned RAG faithfulness 检测 benchmark，量化并诊断检测器的失效边界。**

一句话卖点：**填补中文医学 faithfulness 空白 + 诊断现成检测器医学域失效边界 + 给出诊断性基线**。

### ⚠️ 工作假设更新（2026-07-11 K3 初验 → 用户拍板「先稳一步」，措辞留活口）

**背景**：原 headline 写「现成检测器在医学域**系统性失效**」。K3 命门用 4 个跨家 LLM-judge 在 28 条平衡集上初验，揭示失效是**规模/能力依赖**而非无条件系统性——中小 judge（GLM-4-9B 0.14 / Hunyuan-13B 0.29 recall_unfaithful≈随机）失效，但强 judge（MiniMax 0.79 / GLM-4.5-Air 0.71）抓得住大部分、仍漏 20-30%（逐 judge 结果 `code/results/k3_*`）。

**当前工作假设（待 P3 大样本严格验证后再锁死最终措辞）**：
> 中小模型 judge（部署常用档）+ NLI 族在中文医学 faithfulness 上不可靠；检测能力随模型规模显著提升，但**即便最强 judge 仍漏 20-30% 幻觉**——faithfulness 在中文医学域始终未被现成检测器可靠覆盖。

- 这版更 nuanced、更真、更难被审稿人攻，契合 MedHallu「大模型 judge 更好」的已知规律，且正是 skeptic K1 判据预埋的分支（实验↔叙事互调按设计发生，非翻车）。
- **不锁死程度词**：headline 用「不可靠 / 失效边界」而非「系统性失效」；最终措辞（是否强调规模曲线、漏检率具体区间）等 **P3 大样本（D2-D15 全谱 + 放量 + bootstrap CI）** 回来定。
- **caveat（诚实天花板）**：28 条小样本噪声大；幻觉是「骗过≥1 投票器」的对抗构造样本，构造强度对失效有份；单点 recall 非 P3 正式区间估计。
...
>
> **#1 内部失效程度轴（2026-07-11 K3 后新增）**：headline #1 的「检测器失效」措辞按 P3 数据分档——(a) 若 P3 大样本证实强 judge 也大面积失效 → 可回「系统性失效」强表述；(b) 若维持 pilot 的规模依赖形态（强 judge 抓大部分、仍漏 20-30%）→ 用「规模/能力依赖的失效边界」nuanced 表述（当前工作假设，见上「工作假设更新」块）。**pilot 28 条不足以锁死，P3 前一律用 (b) 谨慎版。**
...
- **不 claim 无条件「系统性失效」（2026-07-11 K3 后新增红线）**：pilot 揭示失效**规模/能力依赖**（强 judge 抓得住大部分）。P3 大样本前，检测器失效一律写「中小 judge + NLI 族不可靠 / 最强 judge 仍漏 20-30%」的谨慎版，不写无条件「系统性失效」。程度词等 P3 数据定。
