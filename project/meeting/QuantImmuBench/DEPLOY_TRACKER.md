...
| I13 | **T-SCAPE ★** | `MT_TSCAPE` | ✅ 进 benchmark（旧+官方口径）| 官方权重 + 修 2 个官方 repo bug（peptide 列名小写 + pmhc_im_neo 加载）；MT-only；旧 101 肽负相关 ρ=-0.1386；**官方 130 肽口径 2026-07-01 本地 WSL2 补满 130/130（纠「54.7GB」误解：只用 best_param/pmhc_im_neo 0.53GB 本地跑，详 TOOL_RERUN_STATUS 第13行）**；**DTU/CC BY-NC-ND 4.0 学术非商用·ND 禁衍生发布·pending consent** | CC BY-NC-ND 4.0 | 余嘉 |
...
| I16 | **Seq2Neo** | `MT_Seq2Neo`（连续 0-1）| ✅ 官方口径进 benchmark | **2026-07-01 本地 pip 跑通**（绕过 docker `liuxslab/seq2neo` 三源拉不动死路）：`pip install Seq2Neo`(2.1.1,py3.7+TF2.3,CNN 权重随包)+**本地早有 netMHCpan-4.1/netCTLpan-1.1(pTuneos 自带,免 DTU 申请)**→`seq2neo immuno --mode multiple`(`Pep,HLA` 去星)→`Seq2Neo_official.csv` 1761行 **43补跑肽全覆盖**;抽核 NQRNNVVRN×A66:01=0.735413 一致;merge_official_30 PENDING→0=官方 130 肽口径 30/30 满。详 TOOL_RERUN_STATUS 第19行+04_LOG Entry33;netMHCpan/netCTLpan=DTU pending consent；AFL-3.0 | AFL-3.0 | 余嘉 |
