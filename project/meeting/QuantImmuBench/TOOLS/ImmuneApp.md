...
- 分数类型：连续 0–1（sigmoid）；无内置阈值过滤，全肽出分。**实测每肽×每 allele 一行**（多 allele 各出一行）
- **能否定量免疫强弱**：✅ 是（Immunogenicity_score 连续）← 项目核心目标
- **实测输出**（HPC SMOKE_PASS）：`ImmuneApp_Immunogenicity_predictions.tsv`，列 `Allele / Peptide / Sample / Immunogenicity_score`，例 `HLA-A*01:01 CILGKLFTKK 0.99997`、`ALPPTVYEV 0.00068`
...
- 部署状态：✅ **SMOKE_PASS（2026-06-24 HPC）**。env `envs/immuneapp`（py3.7 + **TF 1.15.0 + keras 2.3.1 + numpy 1.18.5 + h5py 2.10.0 + protobuf 3.20**）。权重随 repo `models/immunogenicity/model_immunogenicity.{json,h5}`。repo 经 github tarball 下载（1.2G，git clone 太慢）。
- **坑（实测）**：①TF1.15 stack 用 `pip install -q` 一次性装会**依赖回溯死循环**（卡 20min+）→ 改**先单装 `tensorflow==1.15` 再装其余**即过。②CPU 跑（`libcuda.so.1` 警告无害）。
- example 烟测：`python ImmuneApp_immunogenicity_prediction.py -f testdata/test_immunogenicity.txt -a 'HLA-A*01:01' 'HLA-A*02:01' 'HLA-B*07:02' -o <out>` → 出 tsv 分数。
- 许可：MIT（自由使用/修改/分发）
