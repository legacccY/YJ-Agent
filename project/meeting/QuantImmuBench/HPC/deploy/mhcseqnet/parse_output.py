"""
parse_output.py — QuantImmuBench §工具部署 P10槽  MHCSeqNet 结果回贴 + 统一 schema
服务项目：quantimmu-bench §工具部署 lever=补呈递组第 10 槽（替 MAAP）

功能（仿 mhcnuggets/parse_output.py，但 MHCSeqNet prob 越高越强 → **不翻转**）：
  1. 读 mhcseqnet_raw.csv（run_mhcseqnet.py 输出）列: peptide, HLA_Allele, prob
  2. 读 universe.csv（34247 行，列 Dataset, Peptide_ID, HLA_Allele, MT_Subpeptide,
     WT_Subpeptide, Window_Size, Position, Elispot）
  3. 建 (peptide, HLA_Allele) → prob 查找表（HLA_Allele 在 raw 中已是带星号原始格式，
     与 universe 直接匹配，无需转换 —— HLA-aware：按 (subpeptide, HLA) 对级查表）
  4. 对 universe 每行：
       MT 分数: (MT_Subpeptide, HLA_Allele) → prob → MT_MHCSeqNet
       WT 分数: (WT_Subpeptide, HLA_Allele) → prob → WT_MHCSeqNet
  5. 方向（重要）：MHCSeqNet prob = binding probability ∈[0,1]，**越高越强（呈递/配体
     似然）→ 不翻转**，直接用。与 benchmark 其他「越大越免疫原」MT_* 列方向一致。
  6. 输出 MHCSeqNet_DS1DS2_scores.csv（34247 行，未匹配/不支持 allele 填 NaN）

输入：
  HPC/deploy/mhcseqnet/mhcseqnet_raw.csv
  scripts/out/newtools/universe.csv

输出：
  scripts/out/newtools/MHCSeqNet_DS1DS2_scores.csv

输出列（固定 schema，严格对齐 MUNIS：4-key + MT/WT 工具列）：
  Dataset, Peptide_ID, HLA_Allele, MT_Subpeptide, MT_MHCSeqNet, WT_MHCSeqNet

  MT/WT_MHCSeqNet = prob（越高越免疫原，**未翻转**）。
...
DEFAULT_OUT = PROJECT_DIR / "scripts" / "out" / "newtools" / "MHCSeqNet_DS1DS2_scores.csv"

# 输出列（固定 schema）
OUTPUT_COLS = [
    "Dataset", "Peptide_ID", "HLA_Allele", "MT_Subpeptide",
    "MT_MHCSeqNet", "WT_MHCSeqNet",
]
...
def build_lookup(raw_df: pd.DataFrame) -> dict:
    """构建 (peptide, HLA_Allele) → prob 查找表（同键取最后一个并计数）。"""
...
def get_score(lookup: dict, peptide: str, allele: str) -> float:
    """查表返回 prob（越高越强，不翻转）。未找到或 NaN → NaN。"""
    NAN = float("nan")
...
def parse(raw_path: pathlib.Path, universe_path: pathlib.Path, out_path: pathlib.Path) -> None:
    ...
    for col in ["peptide", "HLA_Allele", "prob"]:
    ...
    out_df["MT_MHCSeqNet"] = mt_list
    out_df["WT_MHCSeqNet"] = wt_list

    # --- 统计覆盖率 ---
    n_total = len(out_df)
    n_mt = int(out_df["MT_MHCSeqNet"].notna().sum())
    n_wt = int(out_df["WT_MHCSeqNet"].notna().sum())
    print(f"\n[parse] == 覆盖统计 ==")
    print(f"  总行数:                  {n_total}")
    print(f"  MT_MHCSeqNet 非NaN:      {n_mt} ({100*n_mt/n_total:.1f}%)")
    print(f"  WT_MHCSeqNet 非NaN:      {n_wt} ({100*n_wt/n_total:.1f}%)")
    ...
    if n_mt > 0:
        mt_valid = out_df["MT_MHCSeqNet"].dropna()
        print(f"[parse] MT_MHCSeqNet(prob) 统计: min={mt_valid.min():.4f}  max={mt_valid.max():.4f}  median={mt_valid.median():.4f}")

    # --- 方向说明 ---
    print("\n[parse] 方向说明：")
    print("  MT/WT_MHCSeqNet = prob∈[0,1]（binding probability，越高越强呈递/配体似然）。")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="MHCSeqNet 结果回贴 universe 全集（MT+WT 双打分，方向不翻转）"
    )
    parser.add_argument(
        "--raw",
        default=str(DEFAULT_RAW),
        help="mhcseqnet_raw.csv 路径（run_mhcseqnet.py 生成）",
    )
    parser.add_argument(
        "--universe",
    ...
    parser.add_argument(
        ...
        help="输出 MHCSeqNet_DS1DS2_scores.csv 路径",
    )
    ...
    if not raw_path.exists():
        ...
        print("  请先运行: python run_mhcseqnet.py", file=sys.stderr)
        sys.exit(1)
