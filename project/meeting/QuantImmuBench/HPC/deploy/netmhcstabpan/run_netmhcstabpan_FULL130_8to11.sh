#!/bin/bash
#SBATCH --job-name=stabpan_full130_8to11
...
#SBATCH --output=/gpfs/work/bio/jiayu2403/quantimmu/logs/stabpan_full130_8to11_%j.out
#SBATCH --error=/gpfs/work/bio/jiayu2403/quantimmu/logs/stabpan_full130_8to11_%j.err
...
#   ${RAW_CSV} = scripts/out_official/coverage_fix/netmhcstabpan_raw_FULL130_8to11.csv
...
INPUT_DIR=${ROOT}/HPC/deploy/netmhcstabpan/inputs_FULL130_8to11
ALLELE_LIST=${INPUT_DIR}/alleles_FULL130_8to11.tsv
OUT_DIR=${ROOT}/scripts/out_official/coverage_fix/stabpan_out_8to11
RAW_CSV=${ROOT}/scripts/out_official/coverage_fix/netmhcstabpan_raw_FULL130_8to11.csv
...
while IFS=$'\t' read -r allele_safe allele_nmhc allele_star; do
    ...
    # 8-11mer 口径修正: netMHCstabpan -p 肽模式要求单次所有肽等长 → 按长度拆跑。
    # 同时 strip CRLF(Windows 建的 .pep 带 \r 会污染长度判定)。
    clean_pep="${TMPDIR}/${allele_safe}_clean.pep"
    sed 's/\r$//' "$pep_file" > "$clean_pep"
    : > "$out_file"        # 清空聚合输出
    : > "${out_file}.err"
    allele_ok=1
    for L in 8 9 10 11; do
        len_pep="${TMPDIR}/${allele_safe}_L${L}.pep"
        awk -v L=$L 'length($0)==L' "$clean_pep" > "$len_pep"
        [ -s "$len_pep" ] || continue
        if ! "$STABPAN" -a "$allele_nmhc" -l "$L" -p "$len_pep" >> "$out_file" 2>> "${out_file}.err"; then
            echo "  WARN: len=$L failed for ${allele_nmhc}" >&2
            allele_ok=0
        fi
    done
    if [ "$allele_ok" -eq 1 ]; then
        echo "OK: $out_file"
        success_count=$((success_count + 1))
    else
        echo "ERROR: some length failed for ${allele_nmhc} (see ${out_file}.err)" >&2
