# -*- coding: utf-8 -*-
"""
prep_stabpan_FULL130.py — QuantImmuBench 覆盖修复战役
...
映射表 alleles_FULL130.tsv（供 run 脚本遍历）。
...
产物（本目录 inputs_FULL130/，主线上传到 HPC ${ROOT}/HPC/deploy/netmhcstabpan/inputs_FULL130/）：
  ...
  inputs_FULL130/alleles_FULL130.tsv      无表头 3 列 TSV: safe<TAB>nmhc<TAB>star
"""
import pathlib
import pandas as pd
...
OUT_DIR = SCRIPT_DIR / "inputs_FULL130"
...
def main():
    ...
    print(f"[src] m9 rows(DS1+DS2, 非 dedup)={len(m9)}  peptides={npep(m9)}")
    ...
    tsv_path = OUT_DIR / "alleles_FULL130.tsv"
