"""
parse_output.py — QuantImmuBench §Tier-0  Repitope 结果回贴 universe
服务项目：quantimmu-bench §工具扩张v2 lever=部署Repitope apples-to-apples

功能：
  1. 读 repitope_raw.csv（run_repitope.R 输出）
  ...
  3. 建 peptide → ImmunogenicityScore 的查找表（HLA-agnostic：仅按肽序列查）
  4. 对 universe 每行：
       MT 分数: peptide_lookup[MT_Subpeptide]  → MT_Repitope
       WT 分数: peptide_lookup[WT_Subpeptide]  → WT_Repitope
  5. HLA-agnostic 映射：同一肽对所有 HLA_Allele 行填同值
     → 同肽不同 allele 的行 MT_Repitope 值相同（caveat 详见 NOTES.md）
  6. 12-14mer 及 run_repitope.R 未打分的肽 → NaN
  7. 输出 Repitope_DS1DS2_scores.csv（34247 行，NaN 覆盖缺失）

输入：
  HPC/deploy/repitope/repitope_raw.csv       ← run_repitope.R 产生
  scripts/out/newtools/universe.csv

输出：
  scripts/out/newtools/Repitope_DS1DS2_scores.csv
...
方向说明：
  ImmunogenicityScore [0-1]，越高越免疫原。直接作为 MT_Repitope / WT_Repitope 使用，
  无需翻转（Spearman(ρ, ELISpot) 正相关方向正确）。
...
DEFAULT_RAW      = SCRIPT_DIR  / "repitope_raw.csv"
DEFAULT_UNIVERSE = PROJECT_DIR / "scripts" / "out" / "newtools" / "universe.csv"
DEFAULT_OUT      = PROJECT_DIR / "scripts" / "out" / "newtools" / "Repitope_DS1DS2_scores.csv"
...
# ---------------------------------------------------------------------------
# 读 raw（repitope_raw.csv）→ 查找表
# ---------------------------------------------------------------------------

def build_lookup(raw_path: pathlib.Path) -> dict:
    """
    读 repitope_raw.csv，建 peptide → ImmunogenicityScore 字典。
    若同一 Peptide 出现多次（不应发生），取第一条。
    """
    ...
    if "Peptide" not in raw_df.columns:
        ...
        sys.exit(1)
    if "ImmunogenicityScore" not in raw_df.columns:
        print(f"[parse] ERROR: raw CSV 缺 'ImmunogenicityScore' 列，实际列: {list(raw_df.columns)}", file=sys.stderr)
        sys.exit(1)
...
def parse(raw_path: pathlib.Path, universe_path: pathlib.Path, out_path: pathlib.Path) -> None:
    ...
    # 同一肽不同 HLA_Allele 行 → 相同 MT_Repitope 值（此为 HLA-agnostic caveat）
    mt_scores = []
    wt_scores = []
    ...
    out_df["MT_Repitope"] = mt_scores
    out_df["WT_Repitope"] = wt_scores

    # --- 覆盖统计 ---
    n_total = len(out_df)
    n_mt_filled = out_df["MT_Repitope"].notna().sum()
    n_wt_filled = out_df["WT_Repitope"].notna().sum()

    print(f"\n[parse] == 覆盖统计 ==")
    print(f"  总行数:                {n_total}")
    print(f"  MT_Repitope 非NaN:     {n_mt_filled} ({100 * n_mt_filled / n_total:.1f}%)")
    print(f"  WT_Repitope 非NaN:     {n_wt_filled} ({100 * n_wt_filled / n_total:.1f}%)")
    ...
    if n_mt_filled == 0:
        print(
            "[parse] WARNING: MT_Repitope 全为 NaN，请检查 repitope_raw.csv 和 universe 的肽序列对应关系。",
            file=sys.stderr,
        )
    ...
    print("\n[parse] 方向说明：")
    print("  MT_Repitope:  ImmunogenicityScore [0-1]，越高越免疫原，直接使用（无需翻转）。")
    print("  WT_Repitope:  同上，来自 WT_Subpeptide 的独立打分。")
    print("\n[parse] ⚠️ HLA-agnostic caveat：")
    print("  Repitope 只依赖肽序列，不使用 HLA 信息。")
    print("  同一肽对所有 HLA_Allele 行填相同 MT_Repitope / WT_Repitope 值。")
    print("  benchmark 报告须标注此 caveat（见 NOTES.md §HLA-agnostic）。")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Repitope 结果回贴 universe 全集（MT+WT 双打分，HLA-agnostic 映射）"
    )
    parser.add_argument(
        "--raw",
        default=str(DEFAULT_RAW),
        help="repitope_raw.csv 路径（run_repitope.R 生成；默认 HPC/deploy/repitope/repitope_raw.csv）",
    )
    parser.add_argument(
        "--universe",
    ...
    parser.add_argument(
        ...
        help="输出 Repitope_DS1DS2_scores.csv 路径",
    )
    ...
    if not raw_path.exists():
        ...
        print("  请先运行: Rscript run_repitope.R --frag-lib ... --feature-df ...", file=sys.stderr)
        sys.exit(1)
