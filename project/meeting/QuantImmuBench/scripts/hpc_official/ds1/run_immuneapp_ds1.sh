...
#SBATCH --output=/gpfs/work/bio/jiayu2403/quantimmu/ds1/logs/ia_ds1_%j.out
#SBATCH --error=/gpfs/work/bio/jiayu2403/quantimmu/ds1/logs/ia_ds1_%j.err
# =============================================================================
# run_immuneapp_ds1.sh — ImmuneApp on DS1 inputs (DS1 独立人类队列复现集, 全 9mer, CPU)
# 克隆自 run_immuneapp_8to11.sh，唯一改动 = INPUT_BASE→ds1 / OUTPUT_BASE→ds1/immuneapp_out
#   （+ #SBATCH job-name/output/error 改 ds1）。工具调用/超参/长度处理零改动。
# MT + WT 双侧都跑（ds1 全 allele 均含 peps_WT.txt）。DS1 全 9mer，ImmuneApp 自适应长度。
# =============================================================================
set -u

QUANT_BASE=/gpfs/work/bio/jiayu2403/quantimmu
IMMUNEAPP_DIR=${QUANT_BASE}/tools_repos/ImmuneApp
INPUT_BASE=${QUANT_BASE}/ds1
OUTPUT_BASE=${QUANT_BASE}/ds1/immuneapp_out
...
echo "[DONE] ImmuneApp ds1：allele 目录 ${n_done} 个，预测 .tsv 文件=${n_tsv}"
echo "IMMUNEAPP_DS1_DONE"
