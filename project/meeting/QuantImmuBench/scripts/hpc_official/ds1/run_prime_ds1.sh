...
#SBATCH --output=/gpfs/work/bio/jiayu2403/quantimmu/ds1/logs/prime_ds1_%j.out
#SBATCH --error=/gpfs/work/bio/jiayu2403/quantimmu/ds1/logs/prime_ds1_%j.err
# =============================================================================
# run_prime_ds1.sh — PRIME on DS1 inputs (DS1 独立人类队列复现集, 全 9mer, CPU only)
# 克隆自 run_prime_8to11.sh，唯一改动 = INPUT_BASE→ds1 / OUTPUT_BASE→ds1/prime_out
#   （+ #SBATCH job-name/output/error 改 ds1）。工具调用/超参/长度处理零改动。
# MT + WT 双侧都跑（ds1 全 allele 均含 peps_WT.txt）。DS1 全 9mer。MixMHCpred 定位逻辑不变。
# =============================================================================
set -u
...
OUTPUT_BASE=${QUANT_BASE}/ds1/prime_out
...
echo "[DONE] PRIME ds1：allele 目录 ${n_done} 个，out_MT.txt=${n_mt}，out_WT.txt=${n_wt}"
echo "PRIME_DS1_DONE"
