...
#SBATCH --output=/gpfs/work/bio/jiayu2403/quantimmu/rerun8to11/logs/prime_8to11_%j.out
#SBATCH --error=/gpfs/work/bio/jiayu2403/quantimmu/rerun8to11/logs/prime_8to11_%j.err
# =============================================================================
# run_prime_8to11.sh — PRIME on RERUN8TO11 inputs (改动②/③ 8-11mer 可变窗全量重跑, CPU only)
# 克隆自 run_prime_rerun.sh，唯一改动 = INPUT_BASE→rerun8to11 / OUTPUT_BASE→rerun8to11/prime_out
#   （+ #SBATCH job-name/output/error 改 rerun8to11）。工具调用/超参/长度处理零改动。
# MT + WT 双侧都跑（rerun8to11 全 allele 均含 peps_WT.txt = 改动③ 全 WT）。
# 输入已含 8/9/10/11mer 混长，PRIME 从肽自适应长度。MixMHCpred 定位逻辑不变。
# =============================================================================
set -u

QUANT_BASE=/gpfs/work/bio/jiayu2403/quantimmu
PRIME_DIR=${QUANT_BASE}/tools_repos/PRIME
INPUT_BASE=${QUANT_BASE}/rerun8to11
OUTPUT_BASE=${QUANT_BASE}/rerun8to11/prime_out
...
echo "[DONE] PRIME rerun8to11：allele 目录 ${n_done} 个，out_MT.txt=${n_mt}，out_WT.txt=${n_wt}"
echo "PRIME_8TO11_DONE"
