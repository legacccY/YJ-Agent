#!/usr/bin/env bash
# run_hlathena_8to11.sh — HLAthena MSi on 8-11mer 可变窗全量重跑输入（HPC singularity）。
# 服务 quantimmu-bench 改动②/③ 8-11mer 可变窗全量重跑。lever=HLAthena presentation proxy 单列。
# ⚠️ presentation proxy（非免疫原性），横评里单列 baseline。
#
# 克隆自 run_hlathena_official.sh，改动 = 输入/输出路径 official_inputs→rerun8to11：
#   UNIQ→rerun8to11/newtools/uniq_pep_hla.csv, WORK→rerun8to11/hlathena_official,
#   RAW→rerun8to11/hlathena_raw.csv。PATCHED 复用 $BASE/hla_predict_patched.bash（存在则不重建）。
#   唯一非路径改动 = xargs -P 10 → -P 4（bug-fix：-P 10 超订 4-cpu 会崩整个等位，见 Entry 教训）。
#   长度过滤 8-11mer 逻辑逐字不变；工具 invocation/超参零改动。
#
...
#
# 输入：rerun8to11/newtools/uniq_pep_hla.csv (peptide,HLA_Allele[带星],source)
#   —— 同其它 presml 工具的 universe，保证 strict 对齐 backbone。
# 输出：rerun8to11/hlathena_raw.csv (peptide,HLA_Allele[带星],MSi)
...
UNIQ=$BASE/rerun8to11/newtools/uniq_pep_hla.csv
WORK=$BASE/rerun8to11/hlathena_official
JOBS=$WORK/jobs
OUT=$WORK/out
RAW=$BASE/rerun8to11/hlathena_raw.csv
CHUNK=200
mkdir -p "$JOBS" "$OUT"
