#!/usr/bin/env bash
# =============================================================================
# run_predig_8to11.sh — PredIG on 8-11mer 可变窗全量重跑输入（HPC, CPU only, 分块）
# =============================================================================
# 服务: QuantImmuBench 改动②/③ 8-11mer 可变窗全量重跑（lever=PredIG）
#
# 克隆自 rerun/run_predig_rerun.sh，唯一改动 = 输入/输出路径 rerun→rerun8to11：
#   - 输入切到 8-11 版重跑输入 $BASE/rerun8to11/predig_input.csv（34176 数据行，
#     超容器 5000 上限 → 必须分块，34176 行 / 4000 ≈ 9 块）。
#   - 输出统一落 $BASE/rerun8to11/predig_out/predig_out.csv。
#   - 分块 + 拼接逻辑逐字保留（≤4000 行/块，split 保序 + 逐段前置表头 + 按 ls -v 序拼接）。
#     输入是现成 CSV（rerun8to11/predig_input.csv 已由 build_rerun_inputs.py 产出），
#     不用 prep，直接按行切（保表头 + 行序）。
#
...
#   直跑： bash run_predig_8to11.sh          （纯 CPU，无需 GPU）
#   或 sbatch 包一层（下方 #SBATCH 头已备，走 cpudebug 分区）。
#
#SBATCH --job-name=predig_8to11
...
INPUT_CSV="$BASE/rerun8to11/predig_input.csv"                 # 现成输入（34176 数据行），只读
WORKDIR="$BASE/rerun8to11/predig_out/work"                   # singularity 挂载 → /work（放分块/中间 out）
FINAL_OUT="$BASE/rerun8to11/predig_out/predig_out.csv"       # 拼接后最终输出
...
if [ -s "$FINAL_OUT" ]; then
  echo "[DONE] PredIG rerun8to11 输出 $n_out 数据行 → $FINAL_OUT"
