...
# -*- coding: utf-8 -*-
"""
patch_add_mhcnuggets.py — QuantImmuBench G1 工具补齐 19→20/30
==================================================================
服务: quantimmu-bench / lever=G1 工具补齐（合 MHCnuggets 一列进大表）

设计哲学（仿 scripts/patch_add_bigmhc_el.py，单列自然键 join，不重建不丢 HLA-FIX）:
  - base = scripts/out/merged_all_tools_19tools.xlsx 活真源（34247 行 × 61 列，
    已 HLA-FIX，已含 ICERFIRE/NetTepi/BigMHC_EL）。本脚本只贴 MHCnuggets 列。
  - score 源 = HPC/deploy/mhcnuggets/mhcnuggets_raw.csv（列 peptide,HLA_Allele,ic50；
...
================== 方向说明（重要，勿删）==================
  MHCnuggets 原始输出 = binding affinity IC50(nM)，**越低越强结合**。
  本脚本建 score_map 时取负（-ic50）→ 越高越免疫原，与 benchmark 其他
  MT_* 工具「越大越免疫原」约定一致。

================== HLA-FIX（同 bigmhc_el：自然键查 base 自身订正 HLA，不置 NaN）==================
  base 表 P101/P102 行 HLA 已订正；mhcnuggets_raw 在同一 uniq_pep_hla 上跑，
  含订正 (pep,hla) 键 → 按 base 订正 HLA 自然键查表天然命中，无须手动置 NaN。

================== 许可 ==================
  MHCnuggets = JHU Karchin Lab BSD-like 许可，学术可发表 —— **非 DTU pending**，
  不写 PENDING_DTU sidecar。

================== 输出 ==================
  scripts/out/merged_all_tools_20tools.xlsx （期望 34247 行 × 63 列）
    新增列: MT_MHCnuggets, WT_MHCnuggets

================== 跑法（主线串行）==================
  1) python scripts/patch_add_mhcnuggets.py  → merged_all_tools_20tools.xlsx
...
BASE = ROOT / 'scripts' / 'out' / 'merged_all_tools_19tools.xlsx'
RAW = ROOT / 'HPC' / 'deploy' / 'mhcnuggets' / 'mhcnuggets_raw.csv'
OUT = ROOT / 'scripts' / 'out' / 'merged_all_tools_20tools.xlsx'

IC50_COL = 'ic50'        # raw 中的分数列名（coder NOTES 已核）


def load_score_map(raw_path: Path) -> dict:
    """读 mhcnuggets_raw.csv，建 score_map {(pep, hla): -ic50}（取负=越高越强）。"""
    df = pd.read_csv(raw_path, encoding='utf-8')
    df.columns = [c.strip() for c in df.columns]

    for c in ('peptide', 'HLA_Allele', IC50_COL):
    ...
    df = df[(df['peptide'] != '') & (df['HLA_Allele'] != '') & df[IC50_COL].notna()]

    # 取负 = 越高越免疫原；同键最后覆盖
    score_map = dict(zip(zip(df['peptide'], df['HLA_Allele']),
                         (-df[IC50_COL]).astype(float)))
    print(f'[INFO] score_map 读入: {len(score_map)} 条唯一 (pep,hla) 键'
          f'（raw 数据行 {len(df)}，方向 -ic50）', file=sys.stderr)
    return score_map
...
def main():
    ...
    for req in ('HLA_Allele', 'MT_Subpeptide'):
        if req not in m.columns:
            ...
            sys.exit(1)
    for col in ('MT_MHCnuggets', 'WT_MHCnuggets'):
        if col in m.columns:
    ...
    m = lookup_col(m, 'MT_Subpeptide', score_map, 'MT_MHCnuggets')
    if 'WT_Subpeptide' in m.columns:
        m = lookup_col(m, 'WT_Subpeptide', score_map, 'WT_MHCnuggets')
    else:
        print('[INFO] base 无 WT_Subpeptide → 跳过 WT_MHCnuggets', file=sys.stderr)
    ...
    if 'Patient_ID' in m.columns:
        ...
        n_pp = int(pp.sum())
        mt_pp = int(m.loc[pp, 'MT_MHCnuggets'].notna().sum())
        wt_pp = (int(m.loc[pp, 'WT_MHCnuggets'].notna().sum())
                 if 'WT_MHCnuggets' in m.columns else 0)
        print(f'[HLA-FIX] P101/P102 行={n_pp}；MT 非空={mt_pp}，WT 非空={wt_pp}',
              file=sys.stderr)

    print('[LICENSE] MHCnuggets = JHU Karchin Lab BSD-like，可发表；'
          '非 DTU pending → 不写 sidecar。', file=sys.stderr)

    OUT.parent.mkdir(parents=True, exist_ok=True)
    m.to_excel(OUT, index=False, engine='openpyxl')
    new_cols = [c for c in ('MT_MHCnuggets', 'WT_MHCnuggets') if c in m.columns]
