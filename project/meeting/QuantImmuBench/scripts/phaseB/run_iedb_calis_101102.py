# -*- coding: utf-8 -*-
"""
run_iedb_calis_101102.py — Phase B：用订正 HLA 等位重推理 IEDB_Calis（P101/P102）。
...
P102={A02:01,B35:03,B38:01}）。本脚本只从这份派生，绝不读旧 iedb/calis 输入文件。
...
产出: scripts/out/phaseB/IEDB_Calis_101102.csv
      列: bb_idx, MT_IEDB_Calis, WT_IEDB_Calis
方向: score 越高越免疫原（官方原始方向，无翻转）。

用法:
    python run_iedb_calis_101102.py [--smoke N]
...
WORKDIR = ROOT / "scripts" / "out" / "phaseB" / "iedb_calis_work"  # per-allele 临时 in/out
OUT = ROOT / "scripts" / "out" / "phaseB" / "IEDB_Calis_101102.csv"
...
def main():
    ap = argparse.ArgumentParser(description="Phase B IEDB_Calis 重推理 P101/P102")
    ...
    # ── 回贴 bb_idx，写 IEDB_Calis_101102.csv ───────────────────────────────
    def fmt(pep, tag):
