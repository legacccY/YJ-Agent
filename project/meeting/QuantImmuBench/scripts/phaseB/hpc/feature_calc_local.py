...
# FIX 1: 动态 sys.path
# HPC 适配：$HOME 未必等于 work 目录（repo 在 /gpfs/work/.../quantimmu），
# 故允许用环境变量 IMPROVE_SRC 显式覆盖 src 路径；未设时退回原 ~/quantimmu（WSL 行为不变）。
_SRC_PATH = os.environ.get("IMPROVE_SRC") or os.path.join(
    os.path.expanduser("~"),
