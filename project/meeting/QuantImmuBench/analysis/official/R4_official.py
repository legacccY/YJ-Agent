...
输出 (analysis/official/):
  R4_ablation_official.csv  —— long format:
    section=leave_one_out : 每 (ndim,fusion,variant) 一行 (variant=__full__ 为全维基线),
                            含 fusion_rho / delta_vs_full (最负=最承重维)。
    section=weighting     : 每 (ndim,variant=加权方案) 一行, delta_vs_full = vs uniform 等权。

复用旧骨架:
  · 引擎 apply_fusion / per_patient_spearman / _fit_simplex → _official_common
...
from _official_common import (                              # noqa: E402
    ...
    best_pooling_for_tool, pool_col, spearman_np, fisherz_weighted_agg,
    _fit_simplex, MIN_PEP, LABEL_COL, FROZEN_POOLED, ensure_out_dir, r6,
)

# ── 维度集成员 (★ TODO 待袁/朱确认, 同 R3; 各工具取 R2 最优 pooling 列) ──────────
...
def resolve_dim_cols(df, tools, pats, min_pep):
    cols, used = [], []
    for t in tools:
        best_pl, _r, _a = best_pooling_for_tool(df, t, patients=pats, min_pep=min_pep)
        if best_pl is None:
...
def main():
    ...
    for ndim in sorted(ABLATION_TOOLSETS.keys()):
        ...
        for fmethod in FUSION_METHODS_ABLATION:
            ...
            rho_full, cl_f, ch_f, nu_f, _ = per_patient_spearman(
            ...
            rows.append(dict(section="leave_one_out", ndim=ndim, fusion_method=fmethod,
                             ...
                             ci_low=r6(cl_f), ci_high=r6(ch_f), n_pat=int(nu_f),
                             delta_vs_full=0.0))
            ...
            for drop, drop_used in zip(dim_cols, used):
                ...
                rho_m, cl_m, ch_m, nu_m, _ = per_patient_spearman(
                ...
                rows.append(dict(section="leave_one_out", ndim=ndim,
                                 ...
                                 fusion_rho=r6(rho_m), ci_low=r6(cl_m),
                                 ci_high=r6(ch_m), n_pat=int(nu_m),
            ...
            # (B) 加权对比 (仅 fmethod=mean_rank 的 rank 空间; geomean 时也跑, 但加权基于 rank)
            if fmethod == "mean_rank":
                rho_uniform = None
                for scheme in WEIGHT_SCHEMES:
                    s_w = weighted_lopo_fusion(df, dim_cols, scheme, pats,
                                               min_pep=args.min_pep)
                    rho_w, cl_w, ch_w, nu_w, _ = per_patient_spearman(
                    ...
                    rows.append(dict(section="weighting", ndim=ndim,
                                     ...
                                     fusion_rho=r6(rho_w), ci_low=r6(cl_w),
                                     ci_high=r6(ch_w), n_pat=int(nu_w),
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        ...
        f.write("# QuantImmuBench §3.3.2 表7: 维度留一消融 + 加权对比 (本地实证)\n")
        f.write(f"# 输入={Path(args.input).name}; DS2 9 患者; 主集6维SURV6, 7维对照; geomean/mean_rank\n")
