...
- 人 DS2（主分析集）：🔴**2026-06-30 切官方更正数据** `data/OFFICIAL_DO_NOT_TOUCH/ELISPOT_OFFICIAL_Braun2025_MOESM4.xlsx`（只读，唯一标准；In Vitro 130 肽 + Ex Vivo 36 行）；旧 `Elispot_Dataset2.xlsx`（101 肽）已废→ `data/_archive_superseded_20260630/`。现有三步范式结果基于旧 101 肽口径，**待官方 130 肽重跑**。详见 `data/README_DATA_OFFICIAL.md`。
- 鼠 B16F10 / CT26：❌ 仓库完全缺失（袁 md §2.1 要求）。
- nested-LOPO：❌ 缺双层；本地仅 `quantimmune/lopo_eval.py` 单层（见 G3）。

> **⚠️ 投稿前拍板点（口径冲突，必须袁老师 / 朱同学统一）**：
> - **官方更正数据（2026-06-30 唯一标准）**：DS2 In Vitro = **130 肽 / 9 患者**（P101–P110 缺 P103）；旧 101 肽是其过滤子集（官方多 29 肽 = 28 阳/1 阴）。
> - **袁 md 声称口径**：DS2 = **92 突变 / 8 有效病人**（inference 子集，P102 在 inference 中近缺席）。
> - **旧本地口径（HLA-FIX 第 7 版，已作废）**：DS2 = 101 有效肽（90 阳/11 阴）/9 患者→7 有效。
> - 三套口径（官方 130 肽 vs 袁 md 92 突变/8 患者 vs 旧 101 肽）不一致，**直接影响表 1、表 5–10 全部 n 与分母**。投稿前必须由袁老师 / 朱同学书面统一，writer 不得自行选边照写。
