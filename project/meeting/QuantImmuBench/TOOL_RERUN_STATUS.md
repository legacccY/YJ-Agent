...
| 13 | TSCAPE | 免疫原 | **本机 WSL2(tscape env,RTX4070)** | ✅ done | **2026-06-30 本地补跑(纠54.7GB误解,只用 best_param/pmhc_im_neo 945M)**：prep 官方 backbone→1462 unique(MT,HLA)对(Step A pseudo_match "NONE"=0 零过滤,26等位全支持)→Step B inference_csv.py(--inf_type pmhc_im_neo,复用 device patch 版不改码)→merge_tscape→`TSCAPE_official.csv` 1761行0NaN;MT_TSCAPE 0.0088-0.7073;抽核 bb0/bb1=0.069996/0.297936 溯源 output 一致;merge_official_30 并入 **130/130肽 43/43补跑 26等位**;CC BY-NC-ND pending consent |
...
| 19 | Seq2Neo | 免疫原 | **本机 WSL2 seq2neo env(py3.7+TF2.3)** | ✅ done | **2026-07-01 本地 pip 跑通(绕过 docker 死路)**：docker `liuxslab/seq2neo` 三源(直连/daocloud白名单拒/1ms.run)大 layer 全拉不动→改 `pip install Seq2Neo`(2.1.1,10.9MB,清华镜像,CNN 权重随包)。两外部二进制**本地全有不用 DTU 申请**：netMHCpan-4.1(ext_tools)+**netCTLpan-1.1(pTuneos/software/netchop/netctlpan_1_1_executable 早带)**→配 PATH→`seq2neo immuno --mode multiple`(`Pep,HLA` 去星表头)→`cnn_results.csv`(immunogenicity 越高越强)→parse→`Seq2Neo_official.csv` 1761行**43补跑肽全覆盖**(无旧分仅新43肽);抽核 NQRNNVVRN×A66:01=0.735413 溯源一致;merge_official_30 并入 PENDING→0;netMHCpan/netCTLpan=DTU pending consent;AFL-3.0 |
...
| 21 | NeoaPred | 呈递结构 | ⛔ DROPPED | ⛔ 去掉 | **用户 2026-07-07 拍板从工具集去掉**(结构物理最慢/旧仅244有效值)。rerun 不再产 NeoaPred_official.csv;从 merge_official_30 ROSTER 注释(30→29 工具)。已 scancel rerun job1512890 + release slot297c5264。旧记录: job1502935 跑完(gpu4090n9,1h35m,244严格9mer),NeoaPred_official.csv 244非空——**此旧产物保留在 out_official/ 供参考,但不进 rerun 收口**。 |
