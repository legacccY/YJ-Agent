...
# Path in repo: DSCNet_2D_opensource/Code/DRIVE/S3_DSConv_pro.py
# License: MIT (Copyright (c) 2025 Yaolei Qi)
# Fetched: 2026-06-20 (main branch)
# Modifications: device handling made CPU-safe (device param now accepts 'cpu')
...
class DSConv_pro(nn.Module):
    def __init__(
        ...
        Args:
            ...
            morph: the morphology of the convolution kernel is mainly divided into two types
                   along the x-axis (0) and the y-axis (1) (see paper for details).
            if_offset: whether deformation is required. If False, standard convolution kernel.
            device: torch device for coordinate map computations.
        """
    ...
    def forward(self, input: torch.Tensor):
        ...
        y_coordinate_map, x_coordinate_map = get_coordinate_map_2D(
            ...
            device=input.device,  # follow input device (not self.device which may be stale)
        )
...
def get_coordinate_map_2D(
    ...
    """Computing 2D coordinate map of DSCNet.

    Args:
        offset: offset predict by network with shape [B, 2*K, W, H].
        morph: 0 (x-axis snake) or 1 (y-axis snake).
        extend_scope: the range to expand. Defaults to 1.0.
        device: location of data.

    Return:
        y_coordinate_map: shape [B, K_H * H, K_W * W]
        x_coordinate_map: shape [B, K_H * H, K_W * W]
    """
...
def get_interpolated_feature(
    ...
    """Interpolate feature of DSCNet using coordinate maps.

    Args:
        input_feature: shape [B, C, H, W]
        y_coordinate_map: shape [B, K_H * H, K_W * W]
        x_coordinate_map: shape [B, K_H * H, K_W * W]
        interpolate_mode: 'bilinear' or 'bicubic'. Defaults to 'bilinear'.

    Return:
        interpolated_feature: shape [B, C, K_H * H, K_W * W]
    """
    ...
    # grid shape [B, H, W, 2] where [:, :, :, 2] = [x, y]
...
def _coordinate_map_scaling(
    ...
    """Scale coordinate_map from origin=[min,max] to target=[a,b]."""
    min_, max_ = origin
    a, b = target
