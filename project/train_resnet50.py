"""ResNet-50 backbone training for BMVC §5.4 universality study.

Pure backbone classification on ISIC 2020 binary task. Trained once,
val/test logits exported so run_qcts.py can fit (T0, alpha) and run ITB
evaluation with QCTS applied on top of this backbone.

State file (for run-experiment monitor):
  D:/YJ-Agent/log/experiment_state.json

Usage:
    /loop /run-experiment project/train_resnet50.py project/configs/resnet50.yaml

Direct (smoke test):
    python project/train_resnet50.py --config project/configs/resnet50.yaml --dry-run
"""
import argparse
import json
import os
...
# Ensure project root on path when launched from D:/YJ-Agent or D:/YJ-Agent/project
...
def parse_args():
    ...
    p.add_argument("--dry-run", action="store_true",
                   help="One batch forward+backward then exit (smoke test).")
    return p.parse_args()
...
def build_resnet50(num_classes: int, weights_enum: str, dropout: float) -> nn.Module:
    weights = getattr(models.ResNet50_Weights, weights_enum)
...
def cosine_factor(epoch: int, total: int, warmup: int) -> float:
    """Returns a [0, 1] scale factor; multiply by each group's initial_lr."""
    if epoch < warmup:
...
def main():
    ...
    print(f"[train_resnet50] device={device}  config={args.config}  seed={cfg.train.seed}")
    ...
    model = build_resnet50(
        ...
        weights_enum=cfg.backbone.weights,
        dropout=cfg.backbone.dropout,
    ).to(device)
    n_params = sum(p.numel() for p in model.parameters())
    print(f"ResNet-50 params: {n_params/1e6:.1f}M  weights={cfg.backbone.weights}")
    ...
    wandb.init(
        ...
        name=cfg.wandb.get("run_name", "resnet50"),
    )

    state = {
        "status": "running",
        "experiment": {"script": "train_resnet50.py", "config": args.config},
    ...
    for epoch in range(start_epoch, cfg.train.epochs):
        ...
        lr_now = factor * cfg.train.lr  # for logging (head LR)
        ...
        last_path = ckpt_dir / "last_resnet50.pth"
        ...
        if not np.isnan(val_metric) and val_metric > best_metric:
            ...
            best_path = ckpt_dir / "best_resnet50.pth"
    ...
    # Final: load best, export logits for QCTS
    if best_path is not None:
