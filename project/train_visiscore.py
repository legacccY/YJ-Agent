...
_STATE_PATH = Path("D:/YJ-Agent/log/experiment_state.json")


def _update_state(patch: dict):
    """Deep-merge patch into experiment_state.json. No-op if file missing."""
...
def train():
    ...
    if device.type == "cuda":
        ...
        print(f"[GPU] {props.name} | 显存 {props.total_memory // 1024**2} MB")
    else:
        print("[GPU] 无 CUDA，使用 CPU")
    ...
    # 写入 PID，让监测脚本知道进程已启动
    _update_state({"process": {"pid": os.getpid()}})
    ...
    scheduler = None
    if getattr(cfg.train, "lr_scheduler", None) == "cosine":
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer, T_max=cfg.train.epochs - start_epoch, eta_min=1e-6
        )
        print(f"[LR] CosineAnnealingLR, T_max={cfg.train.epochs - start_epoch}")
    ...
    for epoch in range(start_epoch, cfg.train.epochs):
        ...
        # 每 epoch 更新 state.json（监测脚本直接读，不解析日志）
        if _STATE_PATH.exists():
            state_now = json.loads(_STATE_PATH.read_text(encoding="utf-8"))
            history = state_now.get("progress", {}).get("val_metric_history", [])
            history.append(round(avg_plcc, 4))
            _update_state({
                "progress": {
                    "current_epoch": epoch + 1,
                    "total_epochs": cfg.train.epochs,
                    "last_loss": round(avg_train, 6),
                    "last_val_metric": round(avg_plcc, 4),
                    "val_metric_history": history,
                    "last_update_at": datetime.now().isoformat(),
                },
                "checkpoint": {
                    "last_path": str(ckpt_dir / "last_visiscore.pth"),
                    "last_epoch_saved": epoch + 1,
                    **({"best_path": str(ckpt_dir / "best_visiscore.pth")} if avg_plcc > best_plcc else {}),
                },
            })
