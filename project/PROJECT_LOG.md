...
### 🚨 R1 nafnet (1471613) 全 NaN 发散逮捕 + 修复（同会话续，用户授权杀+修+重投）
- **用户质疑「炸了」→ 实测坐实**：金标准扫 best_ft.pth 权重 = **115,982,915 参数 100% 全 NaN**，存 val_psnr=3.19dB 垃圾。首 epoch（10076 iter）就发散。
- **根因链**：amp 默认 True 跑 fp16 + nafnet lr=1e-4 warmup=0 + PSNRLoss `mse.clamp(min=1e-8)` 后 log10——**1e-8 fp16 underflow 成 0 → log10(0)=−inf → loss=−inf**；fp16 forward 溢出 nan → L768 `clamp(0,1)` 不挡 NaN；**run_train_epoch 无 non-finite loss 守护** → nan 写进权重雪崩全网；**L860 `NaN→uint8→0` 静默转换掩盖发散**，拿垃圾 val_PSNR 当 best 存，表面「在跑」49min。
- **教训**：①per-epoch 打印 + `python` 无 `-u` 缓冲 = 瞎跑一小时看不到 ②uint8 cast 静默吞 NaN 是元凶级帮凶 ③**金标准查发散 = 扫 ckpt 权重 isnan 占比**，不信 .out 表面在跑。（强化 [[feedback_debug_silent_failure]]）
- **止血**：scancel 1471613/1471977/1472058（3 个同脚本 job，PENDING 的放出去同样炸）→ gpu_slot release 三条清账（HPC 满→2/4）。
- **coder 修 6 条**：①non-finite loss guard（nan/inf loss zero_grad+skip+计数，绝不 backward）②PSNRLoss/Charbonnier forward `.float()` 上浮 fp32 消 underflow ③forward NaN 早检不 nan_to_num 掩盖 ④val+eval cast 前 finite 检查、全坏返 nan、计 n_bad ⑤best-save 加 `math.isfinite(val_psnr)` 闸 ⑥强化 smoke=30 真步 assert skipped=0 + 权重全 finite（旧 smoke=1 步抓不到）。主线补：每 200 iter 心跳打印 + submit 加 `python -u` + `--no_amp`（fp32 彻底消 fp16 风险，用户「不卡时间」correctness 优先）。
- **重投 1 个 nafnet 验证**：job 1472609（gpu_slot 38f4bba0），SLURM PENDING 等卡（全校满）。**staged：先这 1 个看心跳 loss finite + PSNR 爬 20-30 → 才铺其余 5 baseline + R3，不再盲投**。

### 待续
1. **1472609 起跑 → 看 `[hb]` 心跳**：loss finite + train_PSNR 朝 20-30 爬 + skipped=0 = 真数据管线 sane → gpu_slot 申请铺 mirnetv2/swinir/uformer/realesrgan + R3 graft（各 `--no_amp`）。仍 NaN → 进一步查 lr/数据归一。
...
### ⚠️ autonomous tick：R1/R3 被新窗 scancel 转 selinf + restormer OOM 修复（2026-06-19 晚）
- **R1/R3 全停**（sacct 核）：nafnet `CANCELLED by 2853`@64min（跑了 64min 无 csv）、restormer `COMPLETED`@45s（实为 OOM 崩）、R3 `CANCELLED by 2853`@0s。**UID 2853 = jiayu2403 自己** → 用户开新窗做 **selinf（R1 BIBE strengthening，gpu_slot 7724eb16 / SLURM 1472435，user-submitted via `!`）**，scancel 了 ICLR R1/R3 腾卡。**用户主动转 HPC 资源给 selinf，ICLR R1 有意暂停**。autonomous 不抢卡不重提交。
- **restormer OOM 实证修复**（coder，复现零偏离）：重 transformer baseline（restormer/swinir/uformer/mirnetv2/realesrgan + R3）4090 24GB 第一 batch 即 OOM（崩在 restormer_arch FFN）。修=micro_batch=2 × accum_steps=2 = **有效 batch 4 = 官方值（数学等价，非降 batch/步数）** + `PYTORCH_CUDA_ALLOC_CONF=expandable_segments` + grad accum 训练循环（AMP scaler 配合、loss/accum 保梯度尺度、non-finite 整组丢弃）。nafnet（轻 CNN micro=8 不动，已验 64min 无 OOM）。smoke 过。**修好存着待重跑**。
- 提交脚本 `_scratch_r1_submit.py` 被新窗改：加 `python -u`（解 stdout 缓冲盲等）+ `--no_amp`（关 AMP）。
- gpu_slot 死账已被新窗清（release WARN 不匹配=已清）。
- **待用户回来协调**：①多窗口 HPC 共享 jiayu2403 账号（selinf vs ICLR 抢卡，需排期）②决定 ICLR R1/R3 何时重跑（OOM 已修，有空卡即可，但别和 selinf 抢）③重跑用修后脚本（micro-batch+accum 防 OOM）。
