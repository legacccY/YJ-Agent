...
下载 exe 时浏览器或 Windows 弹出安全警告，**这是误报，不是病毒**。根本原因：这是一个学生开源项目，没有购买代码签名证书，新发布的 exe 在 Microsoft SmartScreen 那里零声誉——零声誉就会被当作潜在威胁拦截，和文件本身是否有害无关。

如果你不放心，**最直接的方式是走源码运行**（见下方 [🚀 源码运行](#-源码运行)），本地跑 Python 脚本完全绕开这三层警告，也更方便自己审查代码。
