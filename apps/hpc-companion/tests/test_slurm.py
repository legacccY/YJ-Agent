...
def test_parse_scontrol():
    out = "JobId=123 JobName=test JobState=RUNNING WorkDir=/gpfs/work/x StdOut=/gpfs/work/x/logs/123.out"
    d = slurm.parse_scontrol(out)
    assert d["JobId"] == "123"
    assert d["WorkDir"] == "/gpfs/work/x"
...
def test_build_sbatch():
    spec = slurm.SbatchSpec(
        job_name="exp1", account="shuihuawang", partition="gpu4090",
        qos="4gpus", gpus=4, workdir="/gpfs/work/x",
        command="python train.py")
    s = slurm.build_sbatch(spec)
    assert s.startswith("#!/bin/bash")
    assert "#SBATCH --job-name=exp1" in s
    assert "#SBATCH --gres=gpu:4" in s
    assert "#SBATCH --account=shuihuawang" in s
    assert "cd /gpfs/work/x" in s
    assert "python train.py" in s
    # 默认 output 落到 workdir/logs
    assert "/gpfs/work/x/logs/%j.out" in s
